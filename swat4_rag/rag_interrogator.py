...
def main():
    ...
    sql_base = "SELECT id, category, source_repo, filepath, language, file_type, content FROM rag_data WHERE id=?"
    sql_params = []
    ...
    for i in range(k_search):
        ...
        if row:
            db_id, category, source_repo, filepath, language, file_type, content = row
            results.append({
                "id": db_id,
                "distance": dist,
                "category": category,
