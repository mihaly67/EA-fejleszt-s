...
input string        InpComment           = "TrojanHorse_v1";
...
int OnInit()
  {
   ...
   Print("Trojan Horse v1.0 Initialized. Stealth: ", InpStealthMode);
...
void OnTick()
  {
   ...
   //    Or strictly only when active/paused? Let's allow management when not STOPPED.
...
void Log(string action, ulong ticket, double price, double vol, double profit)
  {
   ...
   FileWrite(g_log_handle, StringFormat("%s.%s,%s,%d,%.5f,%.2f,%.2f", t, ms, action, ticket, price, vol, profit));
  }
