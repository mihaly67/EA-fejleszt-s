//+------------------------------------------------------------------+
//|                                     HybridContextIndicator_v2.9.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 2.9 (Manual Lock Logic & Fibo Color Loop)            |
//+------------------------------------------------------------------+
...
input ENUM_LINE_STYLE    InpSecPivotPStyle     = STYLE_SOLID;

input color              InpSecPivotR1Color    = clrRed;
input int                InpSecPivotR1Width    = 1;
input ENUM_LINE_STYLE    InpSecPivotR1Style    = STYLE_SOLID;

input color              InpSecPivotS1Color    = clrGreen;
input int                InpSecPivotS1Width    = 1;
input ENUM_LINE_STYLE    InpSecPivotS1Style    = STYLE_SOLID;
...
void UpdateFiboNative(const datetime &time[])
{
   ...
   // --- MANUAL LOCK LOGIC ---
   // If the Fibo object exists and is SELECTED by the user, DO NOT update/move it.
   ...
   if(hh_idx != -1 && ll_idx != -1 && hh_idx != ll_idx)
   {
       ...
       // General Properties (Diagonal Line)
       ...
       // Apply styles to EACH level explicitly
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Context v2.9");
