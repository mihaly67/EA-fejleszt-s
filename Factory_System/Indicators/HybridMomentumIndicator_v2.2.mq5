//+------------------------------------------------------------------+
//|                                 HybridMomentumIndicator_v2.2.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 2.2 (Hybrid Signal: Kalman MACD + Lowpass Signal)    |
//+------------------------------------------------------------------+
...
//--- Plot 2: MACD Line (Nonlinear Kalman - FAST)
#property indicator_label2  "MACD (Kalman)"
#property indicator_type2   DRAW_LINE
#property indicator_color2  clrDodgerBlue // BLUE = Fast
#property indicator_style2  STYLE_SOLID
#property indicator_width2  2

//--- Plot 3: Signal Line (Lowpass - SLOW)
...
input int                InpFastPeriod         = 5;      // Fast Period (Kalman)
input int                InpSlowPeriod         = 13;     // Slow Period (Kalman)
input int                InpSignalPeriod       = 6;      // Signal Period (Lowpass)
input ENUM_APPLIED_PRICE InpAppliedPrice       = PRICE_CLOSE; // Applied Price
input double             InpKalmanGain         = 1.0;    // Kalman Gain (MACD)
...
double      sig_lowpass_buffer[]; // Simple Lowpass for Signal
double      vol_ma_buffer[];      // Placeholder, calculated inline/or reusing logic
...
int OnInit()
{
   ...
   // 3. Visuals
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.2 (Refined)");
...
//+------------------------------------------------------------------+
//| Helper: Nonlinear Kalman Update (For MACD Lines)                 |
//+------------------------------------------------------------------+
void UpdateKalman(int i, double price, double period,
                 double &lowpass[], double &delta[], double &output[])
{
    // Corrected Alpha Logic: 2.0 / (N+1)
    // Higher Period -> Smaller Alpha -> Slower
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // --- D. Signal Line (Standard Lowpass) ---
      // Changed from Kalman to Simple Lowpass to avoid overshoot/noise in trigger
      ...
      if(is_weak && InpUseVolumeFilter) {
          ...
          // Standard: Green if MACD > Signal (Hist > 0), Red if MACD < Signal
