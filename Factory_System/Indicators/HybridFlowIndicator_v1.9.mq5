//+------------------------------------------------------------------+
//|                                        HybridFlowIndicator_v1.9.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 1.9 (Hybrid MFI + Delta + VROC - Unbounded Peaks)     |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Flow v1.9");
                ...
                const int &spread[])
{
   ...
   // --- MFI Calculation ---
   // Read MFI into temporary buffer if we modify it, or direct buffer if we overwrite.
   // We will read into MFIBuffer directly, then modify it in place.
   ...
   for(int i = start; i < rates_total; i++)
   {
       ...
       // 4. Fill Delta Buffers (UNBOUNDED for v1.9)
       DeltaStartBuffer[i] = 50.0;
       double final_val = 50.0 + scaled_offset;

       // NO CLAMPING HERE - Allow values to exceed Min/Max visually if scale permits
       DeltaEndBuffer[i] = final_val;
       DeltaColorBuffer[i] = (smooth_delta >= 0) ? 0 : 1;

       // 5. HYBRID MFI LOGIC (Inject Delta into MFI Line)
       // This satisfies "peaks should not be flat at 100".
       // Standard MFI is 0-100. We add the (ScaledDelta - 50) component?
       // No, ScaledDelta is centered at 0 offset (in variable 'scaled_offset').
       // If we add 'scaled_offset' to MFI, we make MFI react to Volume Flow intensity.
       // Example: MFI=95, Delta=+20 -> Result=115. Visible peak!

       MFIBuffer[i] += scaled_offset;

       // 6. VROC Logic (Color the MFI Line)
