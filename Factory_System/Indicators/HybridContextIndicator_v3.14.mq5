//+------------------------------------------------------------------+
//|                                    HybridContextIndicator_v3.14.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 3.14 (Triple ZigZag + Master Switch)                 |
//+------------------------------------------------------------------+
...
#property indicator_color9  clrDarkSlateBlue
...
input int                InpTerDeviation       = 10; // Similar to Slow, maybe higher? Keeping conservative.
input int                InpTerBackstep        = 5;
input color              InpTerColorP          = clrPurple;
input color              InpTerColorR1         = clrMediumVioletRed;
input color              InpTerColorS1         = clrDarkSlateBlue;
input ENUM_LINE_STYLE    InpTerStyle           = STYLE_SOLID;
input int                InpTerWidth           = 2;
...
int OnInit()
{
   ...
   PlotIndexSetInteger(6, PLOT_LINE_COLOR, InpTerColorP);
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Context v3.14");
   ...
   if(InpShowPivots)
   {
      ...
      {
         Print("Error: Could not load required ZigZag indicators.");
         // We don't fail Init because Trends might still work, but we log it.
      }
   }
                ...
                const int &spread[])
{
   ...
   if(InpShowPivots)
   {
      ...
      for(int i = start; i < rates_total; i++)
      {
         ...
         if(micro_zz_handle != INVALID_HANDLE) {
             ...
             MicroR1[i] = m_r; MicroS1[i] = m_s; MicroP[i] = (m_r + m_s + close[i]) / 3.0;
         } else {
         ...
         if(InpUseTertiary && ter_zz_handle != INVALID_HANDLE) {
             ...
             // Search slightly deeper for Tertiary if needed, but 300 should cover Depth 60 swings
             for(int k=i; k > i - search_limit && k >= 0; k--) {
   ...
   {
      // If Pivots disabled, clear buffers or leave empty?
      // Leaving them alone is fine, or set to EMPTY_VALUE if we want to erase them from chart.
      // Better to set to EMPTY_VALUE to clear visuals if user toggles mid-session.
      // But we need to loop. For performance, maybe just let them be stale?
      // User requirement: "ne terhelje a cpu t". So we do NOTHING.
   }
