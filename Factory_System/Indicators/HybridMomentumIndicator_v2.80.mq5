//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.80.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.80 (FIX: Real-Time Feedback Loop Corruption)    |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.80 - KRITIKUS JAVÍTÁS
   ===================================================================
   Ez a verzió javítja a valós idejű "görbe összeomlást" és pirosodást.

   JAVÍTÁS (v2.80):
   A v2.79-ben a Boosted (Stochastical kevert) jel visszaírásra került
   a `raw_macd_buffer`-be. Ez korrumpálta a Standard Deviation számítás
   történelmét, mivel a következő gyertyánál már a módosított jelet
   használta bázisnak. Ez egy pozitív visszacsatolást hozott létre,
   ami tönkretette a normalizálást valós időben.

   MEGOLDÁS:
   - A `raw_macd_buffer` mostantól KIZÁRÓLAG a nyers (Raw) MACD értéket
     tárolja a statisztikai bázishoz.
   - A `final_signal` (Boosted érték) közvetlenül a megjelenítésbe
     és a Signal vonal szűrésébe megy, de NEM írja felül a bázist.
*/
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.80");
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // --- CRITICAL FIX v2.80 START ---
      // STORE RAW MACD for Statistical Baseline.
      // This buffer MUST NOT be overwritten by the Boosted signal!
      raw_macd_buffer[i] = raw_macd;
      // --- CRITICAL FIX v2.80 END ---
      ...
      // FIX v2.80: DO NOT WRITE BACK TO raw_macd_buffer!
      // raw_macd_buffer[i] = final_signal; // <--- REMOVED

      // Signal Update (Feed the Boosted Signal directly)
      UpdateLowpass(i, final_signal, (double)InpSignalPeriod, sig_lowpass_buffer);

      // Final Normalization
      // We normalize the Boosted Signal using the Raw Signal's Noise Floor (StdDev)
      // This keeps the display proportional to volatility.
      // Note: std_dev_macd is the correct noise floor derived from pure history.
