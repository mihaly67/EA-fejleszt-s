//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.82.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.82 (FIX: Group Removal for EA Integration)      |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.82 - INTEGRATION UPDATE
   ===================================================================
   A v2.82-es verzió eltávolítja az `input group` direktívákat
   az `Mimic_Trap_Research_EA` stabilitása érdekében.

   A v2.81 funkciói (Slope Coloring) változatlanok maradnak.
*/
...
//--- Input Parameters
input ENUM_COLOR_LOGIC   InpColorLogic         = COLOR_SLOPE; // [VISUAL] Color Logic Mode

// Momentum Settings
input int                InpFastPeriod         = 3;      // [MOMENTUM] Fast Period
input int                InpSlowPeriod         = 6;      // [MOMENTUM] Slow Period
input int                InpSignalPeriod       = 13;     // [MOMENTUM] Signal Period
input ENUM_APPLIED_PRICE InpAppliedPrice       = PRICE_CLOSE; // [MOMENTUM] Applied Price
input double             InpKalmanGain         = 1.0;    // [MOMENTUM] Kalman Gain
input double             InpPhaseAdvance       = 0.5;    // [MOMENTUM] Phase Advance

// Stochastic Mixing
input bool               InpEnableBoost        = true;   // [STOCH] Enable Stochastic Mix
input double             InpStochMixWeight     = 0.2;    // [STOCH] Mixing Weight (0.0 - 1.0)
input int                InpStochK             = 5;      // [STOCH] Stochastic K
input int                InpStochD             = 3;      // [STOCH] Stochastic D
input int                InpStochSlowing       = 3;      // [STOCH] Stochastic Slowing

// Normalization Settings
input int                InpNormPeriod         = 100;    // [NORM] Normalization Lookback
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.82");
