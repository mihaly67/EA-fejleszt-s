//+------------------------------------------------------------------+
//|                                 HybridMomentumIndicator_v2.4.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|        Verzió: 2.4 (Phase Advance + Stoch Boost Fusion)           |
//+------------------------------------------------------------------+
...
input double             InpFusionWeight       = 0.5;    // Fusion Weight (0.0 - 1.0)
input double             InpLowVolThreshold    = 0.0001; // Volatility Threshold (Points)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.4 (Fused)");
                ...
                const int &spread[])
{
   ...
   double stoch_temp[];
   if(InpUseStochFusion) {
      // Logic: Copy 'Main' buffer (0) from Stochastic indicator
      // We need to copy enough data for the loop
      int to_copy = rates_total - start;
      if(CopyBuffer(hStoch, 0, 0, to_copy, stoch_temp) <= 0) {
         // Fallback if copy fails, don't crash but maybe skip fusion
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      if(InpUseStochFusion && i > 0) {
          // 1. Calculate Volatility (ATR-like or StdDev of Price Change)
          double price_change = MathAbs(close[i] - close[i-1]);
          // Simple Volatility approximation (Smoothed Price Change)
          // If Price Change is VERY small (Low Volatility), we boost.

          bool low_vol_detected = (price_change < InpLowVolThreshold * 100); // Heuristic scaling

          if(low_vol_detected) { // Only apply correction in low volatility if intended, or always if threshold high

             // Get Stoch Value (0-100)
             // CopyBuffer returns array from oldest to newest if using start_pos relative to Now?
             // Wait, CopyBuffer(..., 0, count, ...) copies from Now backwards?
             // No, CopyBuffer copies into array. If we copied 'to_copy' elements,
             // stoch_temp[0] corresponds to 'start' index in the loop...
             // This alignment is tricky. Let's simpler: Use iStochastic directly via function call
             // is slow inside loop.
             // Better: Access the stoch_temp array.
             // stoch_temp size = rates_total - start.
             // Index in stoch_temp = i - start.
             ...
             // Calculate Correction Term (Boost)
             // If Stoch says UP (1.0) -> Add vol_scale * Weight
             double correction = stoch_norm * vol_scale * InpFusionWeight;
