//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.79.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.79 (FIX: EMPTY_VALUE & W1/MN1 Red Bars)         |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.79 - VÉGLEGES STABILIZÁLÁS
   ===================================================================
   Ez a verzió javítja a "piros oszlopok" hibát W1/MN1 idősíkon.

   JAVÍTÁS (v2.79):
   A Stochastic indikátor néha EMPTY_VALUE (vagy nagyon magas) értéket
   ad vissza, ha nincs elég adat (pl. hónapos chart elején).
   A korábbi kód csak a 0.0-t szűrte ki. A v2.79 kiszűri az EMPTY_VALUE-t
   is, így nem "robban fel" a számítás, ami eddig tévesen pirosba
   húzta a hisztogramot.
*/
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.79");
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // FIX v2.78: Fixed Normalization Period
      ...
      if (InpEnableBoost) {
          ...
          if (stoch_idx >= 0 && stoch_idx < ArraySize(stoch_raw_buffer)) {
              ...
              // FIX v2.79: Filter out EMPTY_VALUE (DBL_MAX)
              // MT5 indicators often return EMPTY_VALUE for missing data.
