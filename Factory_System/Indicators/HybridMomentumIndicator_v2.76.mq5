//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.76.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.76 (FIX: Array Series Logic & Buffers)          |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.76 - VÉGLEGES JAVÍTÁSOK
   ===================================================================
   Ez a verzió javítja az "Idő Inverzió" hibát, amit a SetIndexBuffer
   és az ArraySetAsSeries konfliktusa okozott.

   JAVÍTÁSOK (v2.76):
   1. LOGIKAI HIBÁK JAVÍTÁSA: A `stoch_raw_buffer` mostantól egy sima
      dinamikus tömb (nem indikátor puffer), így az `ArraySetAsSeries(true)`
      megbízhatóan működik. Ez biztosítja, hogy a Stochastic adatok
      időben helyesen (0=Legfrissebb) illeszkedjenek a számításhoz.
   2. MEMÓRIA OPTIMALIZÁLÁS: A felesleges indikátor pufferek (13, 14)
      eltávolítva.
   3. MINDEN KORÁBBI JAVÍTÁS TARTALMAZVA:
      - W1/MN1 Sync Check (v2.74)
      - Crash Fix (Bounds Check) (v2.73)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.76");
                ...
                const int &spread[])
{
   ...
   if (InpEnableBoost) {
       // This MUST be called here because stoch_raw_buffer is dynamic, not managed
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      if (InpEnableBoost) {
          ...
          // Since stoch_raw_buffer IS Series (0=Newest), this maps:
          // i=0 (Oldest bar) -> Index = rates_total - 1 (Oldest Stoch).
          // i=rates_total-1 (Newest bar) -> Index = 0 (Newest Stoch).
