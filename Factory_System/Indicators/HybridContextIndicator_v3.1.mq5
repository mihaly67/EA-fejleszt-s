//+------------------------------------------------------------------+
//|                                     HybridContextIndicator_v3.1.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 3.1 (Decoupled Fibo TF, Smart Swing Logic)           |
//+------------------------------------------------------------------+
...
input ENUM_TIMEFRAMES    InpFiboTF             = PERIOD_CURRENT; // Default to Current Chart for Responsiveness
...
//+------------------------------------------------------------------+
//| Helper: Update Fibo (Automated - Smart Swing)                    |
//+------------------------------------------------------------------+
void UpdateFiboNative(const datetime &time[])
{
   ...
   // 1. Get Data from Fibo TF
   int lookback = InpSwingLookback * 2; // Data buffer size
   if (lookback < 50) lookback = 50;    // Minimum buffer to find fractals

   double highs[];
   double lows[];
   datetime times[];

   // Use InpFiboTF instead of InpMicroPivotTF
   ...
   // 2. Smart Swing Detection (Fractal-like: High > Neighbors)
   // We look backwards from the recent completed bar to find the Latest Swing

   int hh_idx = -1;
   int ll_idx = -1;

   // Find latest High Fractal
   // i = count-2 (Last closed bar), down to 2
   for(int i = count - 2; i >= 2; i--)
   {
       if(highs[i] > highs[i-1] && highs[i] > highs[i-2] &&
          highs[i] > highs[i+1] /*&& highs[i] > highs[i+2]*/) // 3-bar or 5-bar fractal
       {
           hh_idx = i;
           break; // Found latest
       }
   ...
   // Fallback: If no fractal found in short lookback, use Absolute Min/Max
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Context v3.1");
