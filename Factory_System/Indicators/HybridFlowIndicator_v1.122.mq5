//+------------------------------------------------------------------+
//|                                     HybridFlowIndicator_v1.122.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 1.122 (FIX: Logic Inversion & Async Stability)      |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID FLOW INDICATOR - v1.122
   ===================================================================
   JAVÍTÁSOK (v1.122):
   1. LOGIC INVERSION FIX:
      - A `RawMFIBuffer` kivéve a `SetIndexBuffer` alól.
      - Így az `ArraySetAsSeries(true)` megfelelően működik, és a
        CopyBuffer a helyes sorrendben (Newest=0) tölti fel az adatokat.
      - Megszűnt a "jövőbe látó" hiba és a fordított adatolvasás.

   2. ASYNC STABILITY:
      - Beépített védelem a történeti adatok betöltésekor (History Load).
      - Ha a letöltött adat (CopyBuffer) kevesebb, mint a `rates_total`,
        az indikátor várakozik (`return 0`), elkerülve a törött görbéket.

   ===================================================================
   ÉRTELMEZÉS (VÁLTOZATLAN):
   - KÉK GÖRBE (Hybrid MFI): MFI + Delta Spread (Nyomás) kombinációja.
   - SIGNAL (Delta Spread): A hisztogramok (Zöld/Piros) a volumen alapú
     vételi/eladási nyomást mutatják.
*/

#property indicator_separate_window
#property indicator_buffers 10 // Reduced from 11 (Removed RawMFI from buffers)
...
double      HybridMFIBuffer[];// (Index 7) - Moved up
...
int OnInit()
{
   ...
   // Note: RawMFIBuffer is removed from SetIndexBuffer to allow Series indexing

   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Flow v1.122");
                ...
                const int &spread[])
{
   ...
   // 1. Set Series Indexing (0 = Newest) for correct CopyBuffer alignment
   ArraySetAsSeries(RawMFIBuffer, true);

   // 2. Copy Data
   int res = CopyBuffer(mfi_handle, 0, 0, rates_total, RawMFIBuffer);

   // 3. Validation: Check if enough data was copied
   if(res <= 0) return 0;
   if(res < rates_total - 5) {
       // Not enough history available yet (Async Loading)
       return 0;
   }
   ...
   for(int i = start; i < rates_total; i++)
   {
       ...
       // 5. HYBRID MFI LOGIC
       // Logic: RawMFIBuffer is Series (0=Newest).
       // Loop 'i' is Standard (0=Oldest).
       // Map 'i' to Series Index: rates_total - 1 - i
       int series_idx = rates_total - 1 - i;

       double raw_mfi_val = 50.0; // Default fallback
       ...
       HybridMFIBuffer[i] = hybrid_val; // For Data Window
