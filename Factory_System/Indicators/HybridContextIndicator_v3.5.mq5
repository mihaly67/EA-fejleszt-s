//+------------------------------------------------------------------+
//|                                     HybridContextIndicator_v3.5.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 3.5 (Full Buffer Visualization for All Pivots)       |
//+------------------------------------------------------------------+
...
// 3 Tiers * 3 Lines (P, R1, S1) = 9 Buffers
// + 2 Trends = 11 Buffers
...
bool GetPivotData(datetime time, ENUM_TIMEFRAMES tf, double &P, double &R1, double &S1)
{
   ...
   // Rolling Logic: Use the immediately preceding completed bar
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Context v3.5");
                ...
                const int &spread[])
{
   ...
   if(InpShowPivots)
   {
      for(int i = start; i < rates_total; i++)
      {
         ...
         {
             if(GetPivotData(time[i], current_tf_sec, P, R1, S1))
             {
         ...
         {
             if(GetPivotData(time[i], current_tf_ter, P, R1, S1))
             {
...
void OnDeinit(const int reason)
{
   ...
   // No objects for Pivots anymore - all buffers!
