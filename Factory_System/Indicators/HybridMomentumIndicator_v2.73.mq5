//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.73.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.73 (FIX: Crash & W1/MN1 Red Bars)               |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.73 - JAVÍTÁSOK
   ===================================================================
   Ez a verzió javítja a "piros hisztogram" hibát magas idősíkokon (W1, MN1)
   és a v2.72-ben tapasztalt fagyást.

   JAVÍTÁSOK:
   1. BIZTONSÁGOS TÖMBINDEXELÉS: A Stochastic adatlekérés (CopyBuffer)
      eredménye mostantól szigorúan ellenőrzött. Ha a Stochastic adatok
      késnek (async loading), az indikátor nem próbál nem létező indexre
      hivatkozni (ami korábban 0-t adott vissza -> fals eladási jel -> piros sáv).
   2. SZINKRONIZÁCIÓ: A `BarsCalculated` használata biztosítja, hogy
      csak validált adatokkal számoljunk.
   3. CRASH FIX: Megszüntetve a "részleges másolás" (partial copy) optimalizáció,
      ami tömbméret-konfliktusokat okozott. Helyette "Smart Full Copy" van,
      ami biztonságos és gyors.
*/
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.73");
                ...
                const int &spread[])
{
   ...
   // --- Fetch Stochastic Data with Safety Checks ---
   if (InpEnableBoost) {
       // 1. Check if Stoch Calculated
       int bars_ready = BarsCalculated(hStoch);
       if (bars_ready < rates_total) {
           // Not enough data calculated yet? Return and retry next tick.
           // Exception: If bars_ready is reasonable (e.g. > 100), we can proceed partially,
           // but safer to return 0 to force sync if we want perfect alignment.
           // However, blocking here can cause 'waiting for data' forever if rates_total is anomalously high.
           // Let's copy what we can.
       }

       ArraySetAsSeries(stoch_raw_buffer, true);

       // 2. Full Copy for Safety (Fixes Index Out of Bounds Crash)
       // We copy ALL available data to ensure the Series Indexing (0=Newest) aligns with 'rates_total'.
       // Only copying 'to_copy' elements works IF we know exactly how to map indices,
       // but on W1/MN1, alignment issues cause the "Red Bar" bug.
       int res = CopyBuffer(hStoch, 0, 0, rates_total, stoch_raw_buffer);

       if (res <= 0) return 0; // Failed to copy, retry next tick
   }
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      if (InpEnableBoost) {
          ...
          // SAFE GUARD:
          // Check if index is valid in the Copied Buffer.
          // If stoch_idx is outside bounds, we assume Neutral (50.0) or skip boost.
          if (stoch_idx >= 0 && stoch_idx < ArraySize(stoch_raw_buffer)) {
              double stoch_val = stoch_raw_buffer[stoch_idx];

              // FIX for Red Bar Bug:
              // If Stoch data is 0.0 exactly (likely empty/uncalculated), ignore it!
              // Stochastic usually ranges 0-100, but exact 0.0 often means "no data".
              if (stoch_val != 0.0) {
