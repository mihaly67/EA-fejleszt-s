//+------------------------------------------------------------------+
//|                                HybridMomentumIndicator_v2.78.mq5 |
//|                     Copyright 2026, Jules Agent & User           |
//|        Verzió: 2.78 (FIX: Stable Normalization - No Adaptive)    |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MOMENTUM INDICATOR v2.78 - STABILIZÁLÁS
   ===================================================================
   Ez a verzió véglegesen megszünteti a "Repainting" (átrajzolás) hibát
   idősík-váltáskor.

   JAVÍTÁS (v2.78):
   A korábbi verziókban volt egy "Adaptív Normalizálás" logika, ami
   a rates_total (teljes gyertyaszám) alapján változtatta a normalizálási
   periódust. Ez azt okozta, hogy ha több history töltődött be (pl.
   idősík váltáskor), az EGÉSZ grafikon megváltozott, mert a normalizálás
   alapja (a periódus hossza) megváltozott.

   A v2.78-ban ez ki lett véve. A normalizálás mostantól mindig a fix
   InpNormPeriod (alapértelmezett 100) értéket használja.
   Ez garantálja, hogy a múltbeli értékek SOHA nem változnak meg attól,
   hogy a jelenben mi történik vagy mennyi adatunk van.
*/
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v2.78");
                ...
                const int &spread[])
{
   ...
   // --- HISTORY CONSISTENCY CHECK (v2.75) ---
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // FIX v2.77: Initialize buffer with RAW value BEFORE CalculateStdDev!
      raw_macd_buffer[i] = raw_macd;

      // FIX v2.78: REMOVED Adaptive Logic.
      // Always use the user-defined InpNormPeriod.
      // This ensures stability regardless of rates_total (history size).
      int current_norm_period = InpNormPeriod;

      // Now CalculateStdDev will read the correct raw_macd_buffer[i]
