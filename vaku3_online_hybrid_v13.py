    ...
    def process_message(self, message):
        ...
        if cmd == "HISTORY_START":
            ...
            if hasattr(self, 'tmp_times'):
                ...
                # Align DOM arrays with zeros for historical data
                if not hasattr(self.dashboard, 'history_av1'):
                    self.dashboard.history_av1 = []
                    self.dashboard.history_av2 = []
                    self.dashboard.history_bv1 = []
                    self.dashboard.history_bv2 = []

                zeros = [0] * len(self.tmp_times)
                self.dashboard.history_av1.extend(zeros)
...
class VakuDashboardOnline(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Vaku 3.0 ONLINE MT5 Bridge (HMM + DOM Monitor)")
    ...
    def setup_ui(self):
        ...
        # --- DOM MONITOR PANEL ---
        self.lbl_dom_title = QLabel("📊 ÉLŐ DOM & SPOOFING MONITOR")
        self.lbl_dom_title.setStyleSheet("font-size: 14px; font-weight: bold; margin-top: 15px; color: #444;")
        adv_layout.addWidget(self.lbl_dom_title)

        # Imbalance Gauge
        self.lbl_imbalance = QLabel("Order Book Imbalance: Várakozás...")
        self.lbl_imbalance.setStyleSheet("font-size: 13px; font-weight: bold; padding: 5px; background-color: #e6e6e6;")
    ...
    def add_live_tick(self, unix_ms, price, pos_type=0, pos_price=0.0, pos_profit=0.0, av1=0, av2=0, bv1=0, bv2=0, ap1=0.0, ap2=0.0, bp1=0.0, bp2=0.0):
        ...
        # DOM buffers init if not exist
        if not hasattr(self, 'history_av1'):
            self.history_av1 = []
            self.history_av2 = []
            self.history_bv1 = []
            self.history_bv2 = []
        ...
        while len(self.history_times) > 0 and self.history_times[0] < cutoff:
            ...
            if hasattr(self, 'history_av1') and len(self.history_av1) > 0:
                self.history_av1.pop(0)
        ...
        try:
            if hasattr(self, 'lbl_adv_time'):
                ...
                if p_buy > thresh:
                    ...
                    # --- DOM MONITOR UPDATE ---
            if hasattr(self, 'history_av1') and len(self.history_av1) > 10:
                av1 = self.history_av1[-1]
                av2 = self.history_av2[-1]
                bv1 = self.history_bv1[-1]
                bv2 = self.history_bv2[-1]
