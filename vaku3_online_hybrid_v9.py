...
class MT5SocketBridge(threading.Thread):
    ...
    def process_message(self, message):
        ...
        if cmd == "HISTORY_START":
            ...
            # Ez történik, amikor nyers történeti adat érkezik (time_msc|bid|ask) a HISTORY_START után
            if len(parts) == 3:
                try:
...
class VakuDashboardOnline(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.p1 = self.graph_widget.addPlot(title="ÉLŐ TICK ÁRFOLYAM (BTCUSD)")
        self.p1.showGrid(x=True, y=True, alpha=0.3)
        self.curve_price = self.p1.plot(pen=pg.mkPen('w', width=2))

        self.graph_widget.nextRow()
        self.p2 = self.graph_widget.addPlot(title="PIACI REZSIM (Makro ER & HMM Kockázat)")
