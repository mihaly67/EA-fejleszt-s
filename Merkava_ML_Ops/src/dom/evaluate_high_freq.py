...
def run_matrix():
    ...
    periods = [7]
    # Mivel 1 perces az idősík, az ATR is kisebb, ezért a 0.5 - 1.5x szorzók jelentik az 1-2 dolláros célokat
    multipliers = [0.5, 1.0, 1.5]
    lookahead = 15 # 15 x 1 perc = 15 perces predikciós ablak (mint a korábbi 3x5 perc)
    
    closes = df_raw["Bar_Close"].values
    
    results = []
    print("START M1 HIGH-FREQ HORIZON MATRIX (15m predikciós ablak)", flush=True)
    ...
    for period in periods:
        ...
        for mult in multipliers:
            ...
            if "Dist_Ctx_EMA_50" in df_raw.columns: features.append("Dist_Ctx_EMA_50")
            if "Dist_Ctx_EMA_150" in df_raw.columns: features.append("Dist_Ctx_EMA_150")
            if "RSI_M5" in df_raw.columns: features.append("RSI_M5")
            if "MACD_M5" in df_raw.columns: features.append("MACD_M5")
            ...
            # Kiszámoljuk hány "napnyi" adat van a teszt halmazban (M1 esetén napi 1440 gyertya)
            total_days_in_test = (len(df_raw) * 0.2) / 1440.0
