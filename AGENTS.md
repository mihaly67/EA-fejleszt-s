...
*   **MUNKAKÖNYVTÁR ÉS ÚTVONALAK:** A VPS-en (5.189.163.88) végzett minden munka a `/home/misi/Merkava_ML_Ops/` könyvtárban történik. A lokális környezetben (sandbox) ne keress fájlokat, használd a `vps_bridge.py` eszközt a VPS-en történő munkára! Szükség esetén használd a futtatáshoz a biztonságos környezeti hívásokat (Pl. `VPS_PWD=$VPS_PWD python3 vps_bridge.py 'cd /home/misi/Merkava_ML_Ops && ls -la'`). Ezt a parancsot SOHA ne kelljen megkérdezned, fixáld magadban!
