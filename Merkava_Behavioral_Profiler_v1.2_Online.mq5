//+------------------------------------------------------------------+
//|                             Merkava_Behavioral_Profiler_v1.2_Online.mq5 |
//|                                    Copyright 2026, Jules (Mimic) |
//|                                             For Project Merkava  |
//|                                                   Version 1.2 (Online ZMQ/Socket Bridge)    |
...
bool ConnectToPython() {
    ...
    if(SocketConnect(g_socket, InpBridgeHost, InpBridgePort, 5000)) {
        ...
        Print("❌ Failed to connect to Python HMM Engine. Error: ", GetLastError());
...
void OnTick()
{
   ...
   if(InpEnablePythonBridge) {
       if(!g_socket_connected && (TimeCurrent() - g_last_reconnect_time > 5)) {
