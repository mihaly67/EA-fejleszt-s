//+------------------------------------------------------------------+
//|                                            DataMiner_NavSystem_v1_00.mqh |
//|                                    Copyright 2026, Jules (Mimic) |
//|                                             For Project Merkava  |
//|                                                   Version 1.00 (DataMiner Custom)   |
//|                   (Visual Toggle Functionality for Naked Chart)  |
//+------------------------------------------------------------------+
...
struct ContextParams {
    ...
    // Trends
    int tr_fast; int tr_medium; int tr_slow; int tr_super; ENUM_MA_METHOD tr_method;
};

class CDataMiner_NavSystem
{
private:
   int      m_handle_rsi;
   ...
   double   m_val_tr_fast, m_val_tr_medium, m_val_tr_slow, m_val_tr_super;
   ...
   string   m_symbol;
   bool     m_visual_active;

public:
   CDataMiner_NavSystem()
   {
      ...
      m_val_tr_fast = 0; m_val_tr_medium = 0; m_val_tr_slow = 0; m_val_tr_super = 0;
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.28)
       PrintFormat("NavSystem: Loading Context (v3.28) from %s", ctx.path);
       m_handle_context = iCustom(symbol, period, ctx.path,
           ...
           ctx.tr_fast, ctx.tr_medium, ctx.tr_slow, ctx.tr_super, ctx.tr_method
       );
   ...
   {
       if(!m_visual_active) return; // Do not attach if visual is off
   ...
   void RemoveFromChart(long chart_id)
   {
       ...
       for(int w=windows-1; w>=0; w--) {
           ...
           for(int i=total-1; i>=0; i--) {
               ...
               string nlow = name; StringToLower(nlow);
               if(StringFind(nlow, "hybrid") >= 0 || StringFind(nlow, "pulse") >= 0 || StringFind(nlow, "flow") >= 0 || StringFind(nlow, "context") >= 0 || StringFind(nlow, "momentum") >= 0)
                   ChartIndicatorDelete(chart_id, w, name);
           }
       }
       ChartRedraw(chart_id);
   }

   void ToggleVisual(long chart_id, bool is_visual_on)
   {
       m_visual_active = is_visual_on;
       if(m_visual_active) AttachToChart(chart_id);
       else RemoveFromChart(chart_id);
   }

   void Refresh(string symbol, MqlTick& latest_tick, datetime current_time = 0)
   {
      // Calculate historical shift based on the tick time
      int shift = 0;
      if (current_time > 0) {
         shift = iBarShift(symbol, PERIOD_CURRENT, current_time);
         if (shift < 0) shift = 0;
      }

      // 1. Data Prep
      int copied = CopyRates(symbol, PERIOD_CURRENT, shift, m_lookback, m_rates);
      if(copied < 100) return;

      m_rates[copied-1].close = latest_tick.bid; // Realtime close (or historical tick bid)
      ...
      if(m_handle_hybrid_macd != INVALID_HANDLE) {
          ...
          if(CopyBuffer(m_handle_hybrid_macd, 0, shift, 1, buf_macd) > 0) m_val_macd = buf_macd[0];
          if(CopyBuffer(m_handle_hybrid_macd, 2, shift, 1, buf_df) > 0) m_val_dfcurve = buf_df[0];
      }

      // Flow (using HybridFlowIndicator_v1.126 buffers: 0=MFI, 2=Delta End, 4=VROC)
      if(m_handle_flow != INVALID_HANDLE) {
          double b[1];
          if(CopyBuffer(m_handle_flow, 0, shift, 1, b)>0) m_val_flow_mfi = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_flow, 2, shift, 1, b)>0) m_val_flow_delta = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_flow, 4, shift, 1, b)>0) m_val_flow_roc = (b[0]==EMPTY_VALUE)?0:b[0];
      } else {
          CalcHybridFlow(copied, shift);
      }

      // Context - MAPPING FOR v3.28
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.28
          if(CopyBuffer(m_handle_context, 0, shift, 1, b)>0) m_val_mic_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 1, shift, 1, b)>0) m_val_mic_s = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 10, shift, 1, b)>0) m_val_mic_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc

          if(CopyBuffer(m_handle_context, 2, shift, 1, b)>0) m_val_sec_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 3, shift, 1, b)>0) m_val_sec_s = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 11, shift, 1, b)>0) m_val_sec_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc

          if(CopyBuffer(m_handle_context, 4, shift, 1, b)>0) m_val_ter_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 5, shift, 1, b)>0) m_val_ter_s = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 12, shift, 1, b)>0) m_val_ter_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc

          if(CopyBuffer(m_handle_context, 6, shift, 1, b)>0) m_val_tr_fast = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 7, shift, 1, b)>0) m_val_tr_medium = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 8, shift, 1, b)>0) m_val_tr_slow = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 9, shift, 1, b)>0) m_val_tr_super = (b[0]==EMPTY_VALUE)?0:b[0];
      }

      // Momentum (using Hybrid_Momentum_WPR_Stoch_v1_04 buffers: 1=Stoch_K, 3=WPR)
      if(m_handle_test_mom != INVALID_HANDLE) {
          double b[1];
          if(CopyBuffer(m_handle_test_mom, 1, shift, 1, b)>0) m_val_stoch_k = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_test_mom, 3, shift, 1, b)>0) m_val_wpr = (b[0]==EMPTY_VALUE)?0:b[0];
      }
   }
   ...
   double GetTrendFast() { return m_val_tr_fast; }
   double GetTrendMedium() { return m_val_tr_medium; }
   double GetTrendSlow() { return m_val_tr_slow; }
   double GetTrendSuper() { return m_val_tr_super; }
...
private:
   ...
   void CalcHybridFlow(int total, int shift)
   {
       ...
       if(m_handle_flow != INVALID_HANDLE) {
           ...
           if(s_mfi_handle != INVALID_HANDLE && CopyBuffer(s_mfi_handle, 0, shift, 1, mfi_buf) > 0) {
