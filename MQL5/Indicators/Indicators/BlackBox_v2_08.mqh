...
//|                                             For Project Merkava  |
//|                                                   Version 2.08   |
//|                    (HybridMomentum v2.82 Test Logging)           |
//+------------------------------------------------------------------+
...
public:
   CBlackBox()
   {
      ...
      m_is_active = false;
      // v2.08: Removed Context Fields, Added Test Momentum Fields
      m_headers = "Time,TickMSC,Phase,MimicMode,Verdict,Bid,Ask,Spread,BidVol,AskVol," +
                  ...
                  "Test_Hist,Test_MACD,Test_Signal," + // v2.82 Fields
   ...
   bool Initialize(string symbol, string version)
   {
      ...
      if(m_file_handle == INVALID_HANDLE) { Print("CRITICAL: Failed to create log file: ", m_filename); return false; }

      FileWrite(m_file_handle, m_headers);
      m_is_active = true;
      Print("BlackBox v2.08 Recording to: ", m_filename);
      return true;
   }

   void CloseLog()
   {
      if(m_file_handle != INVALID_HANDLE) { FileClose(m_file_handle); m_file_handle = INVALID_HANDLE; m_is_active = false; }
   }

   void RecordTick(
      long tick_time_msc, string phase, int mimic_mode, string verdict,
      double bid, double ask, double spread, long bid_vol, long ask_vol,
      ...
      double t_hist, double t_macd, double t_sig, // v2.82 Inputs
