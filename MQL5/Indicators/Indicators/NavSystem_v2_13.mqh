...
//|                                             For Project Merkava  |
//|                                                   Version 2.13   |
//|        (Full Integration: Context v3.19 + Momentum + Pulse + Flow) |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.19
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.19 Support)
       PrintFormat("NavSystem: Loading Context (v3.19) from %s", ctx.path);
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.19
