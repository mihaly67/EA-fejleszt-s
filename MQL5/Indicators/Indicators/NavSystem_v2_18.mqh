...
//|                                             For Project Merkava  |
//|                                                   Version 2.18   |
//|        (Integration: Context v3.25 Reorder + Momentum)           |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.25
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.25 Reorder)
       PrintFormat("NavSystem: Loading Context (v3.25 Reorder) from %s", ctx.path);
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      // Context - NEW MAPPING FOR v3.25
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.25 (Reordered)
          // 0: MicroR (Data)
          // 1: MicroS (Data)
          // 2: SecR (Data)
          // 3: SecS (Data)
