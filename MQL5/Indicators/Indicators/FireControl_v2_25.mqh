//+------------------------------------------------------------------+
//|                                              FireControl_v2_25.mqh |
//|                                                      Jules Agent |
//|                                       Part of Merkava Tank Logic |
//|                                                    Version 2.25  |
//|                    (Stealth Engine & Registry v1.08 Integration) |
//|                    (Strict Stealth: Total Silence v2.0)          |
//+------------------------------------------------------------------+
#ifndef FIRECONTROL_V2_25_MQH
#define FIRECONTROL_V2_25_MQH
...
//| v2.25: Strict Stealth - Enforces Empty Comments ("") always.     |
//+------------------------------------------------------------------+
class CFireControl
{
private:
...
public:
   ...
   void ExecuteTrade(ENUM_ORDER_TYPE type, double vol, double price, double sl, double tp, bool deep_stealth)
   {
       ...
       if(deep_stealth && m_registry != NULL) {
           ...
           final_comment = ""; // TOTAL SILENCE - Empty String
       ...
       if(res) {
           if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE || m_trade.ResultRetcode() == TRADE_RETCODE_PLACED) {
               ...
               if(deep_stealth && m_registry != NULL && ticket > 0) {
                   // Register with SILENCE (Empty String) to be consistent with what went to broker
