//+------------------------------------------------------------------+
//|                                             PanelControl_v2_20.mqh |
//|                                                      Jules Agent |
//|                                       Part of Merkava Tank Logic |
//|                                                    Version 2.20  |
//+------------------------------------------------------------------+
#ifndef PANELCONTROL_V2_20_MQH
#define PANELCONTROL_V2_20_MQH
...
//| v2.20: Fix for directional buttons, updated Version Label        |
//+------------------------------------------------------------------+
class CPanelControl
{
private:
   ...
   string ObjBtnCloseProfit; // New Close Profit Button
   string ObjLabelVirtualTP; // New Virtual TP Label
   string ObjEditVirtualTP;  // New Virtual TP Edit
   string ObjLabelVirtualSL; // New Virtual SL Label
   string ObjEditVirtualSL;  // New Virtual SL Edit

   string ObjLabelBalance;
   string ObjLabelEquity;
   string ObjLabelMargin;
   string ObjLabelFreeMargin; // New Free Margin
   string ObjLabelMarginLevel;
   string ObjLabelTotalPL;    // New Total P/L (Hist)
   string ObjLabelSessionPL;  // New Session P/L
...
public:
   ...
   void Init(string prefix, int x, int y, color bg, color txt,
             ...
             double def_vtp, double def_vsl) // Added VTP/VSL defaults
   {
      m_prefix = prefix;
      m_x = x; m_y = y;
      m_bg_color = bg; m_txt_color = txt;
      // Increased height for Stats (380 -> 500)
   ...
   void Create()
   {
       ...
       ObjectSetString(0, ObjStat, OBJPROP_TEXT, "MERKAVA v2.20"); // Updated
