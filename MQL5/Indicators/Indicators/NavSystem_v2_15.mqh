...
//|                                             For Project Merkava  |
//|                                                   Version 2.15   |
//|        (Integration: Context v3.21 - Cloned from v3.18 Logic)    |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.21
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.21) - LINEAR PARAMS, CLONED FROM v3.18
       PrintFormat("NavSystem: Loading Context (v3.21) from %s", ctx.path);
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.21 (Same as v3.18)
