...
//+------------------------------------------------------------------+
//| Forensic Recorder - CSV Logging (v2.06)                          |
//| Core Improvement: Precision Logging & Context Integration        |
//+------------------------------------------------------------------+
class CBlackBox
{
private:
...
public:
   CBlackBox()
   {
      ...
      m_is_active = false;
      // v2.06 Unified Headers (Will be updated in next steps)
      m_headers = "Time,TickMSC,Phase,MimicMode,Verdict,Bid,Ask,Spread,BidVol,AskVol," +
                  ...
                  "Mic_P,Mic_R,Mic_S,Sec_P,Sec_R,Sec_S,Ter_P,Ter_R,Ter_S,Trend_Fast,Trend_Slow," + // v2.16 Context
   ...
   bool Initialize(string symbol, string version)
   {
      ...
      m_is_active = true;
      Print("BlackBox v2.06 Recording to: ", m_filename);
      return true;
   }
   ...
   void RecordTick(
      ...
      double tr_f, double tr_s, // v2.16 Context
   ...
   )
   {
      ...
      string row = StringFormat(
         ...
         "%.5f,%.5f," +
         "%.5f,%.5f,%.5f," +
         "%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f,%.5f," + // 11 Context Fields
         "%.2f,%.2f,%.2f,%.2f,%.2f,%.2f," +
         "%d,%s,%.2f,%s,%s,%s",
