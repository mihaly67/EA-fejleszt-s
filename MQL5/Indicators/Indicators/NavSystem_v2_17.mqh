...
//|                                             For Project Merkava  |
//|                                                   Version 2.17   |
//|        (Direct Integration: Context v3.18 Reference Logic)       |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.18 (Direct)
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.18 Direct)
       PrintFormat("NavSystem: Loading Context (v3.18 Reference) from %s", ctx.path);

       // Explicit Debug Logging for Context Params
       PrintFormat("Context Params: Pivots=%s Trends=%s Hist=%d Fibo=%s FiboHist=%d",
           (string)ctx.show_pivots, (string)ctx.show_trends, ctx.max_hist, (string)ctx.show_fibo, ctx.fibo_hist);
       PrintFormat("Micro: Use=%s D=%d Dev=%d B=%d Style=%d Width=%d R=%d S=%d",
           (string)ctx.m_use, ctx.m_depth, ctx.m_dev, ctx.m_back, (int)ctx.m_style, ctx.m_width, (int)ctx.m_c1, (int)ctx.m_c2);

       m_handle_context = iCustom(symbol, period, ctx.path,
           ctx.show_pivots, ctx.show_trends, ctx.max_hist,
           false, 0, // Fibo Forced OFF in EA to avoid conflicts
       ...
       if(m_handle_context == INVALID_HANDLE) {
           Print("CRITICAL ERROR: Failed to load Context Indicator v3.18! Error Code: ", GetLastError());
       }
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.18
          // 0=MicP(Calc), 1=MicR, 2=MicS
          // 3=SecP(Calc), 4=SecR, 5=SecS
          // 6=TerP(Calc), 7=TerR, 8=TerS
          // 9=TrF, 10=TrS
          if(CopyBuffer(m_handle_context, 0, 0, 1, b)>0) m_val_mic_p = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 1, 0, 1, b)>0) m_val_mic_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 2, 0, 1, b)>0) m_val_mic_s = (b[0]==EMPTY_VALUE)?0:b[0];

          if(CopyBuffer(m_handle_context, 3, 0, 1, b)>0) m_val_sec_p = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 4, 0, 1, b)>0) m_val_sec_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 5, 0, 1, b)>0) m_val_sec_s = (b[0]==EMPTY_VALUE)?0:b[0];

          if(CopyBuffer(m_handle_context, 6, 0, 1, b)>0) m_val_ter_p = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 7, 0, 1, b)>0) m_val_ter_r = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 8, 0, 1, b)>0) m_val_ter_s = (b[0]==EMPTY_VALUE)?0:b[0];

          if(CopyBuffer(m_handle_context, 9, 0, 1, b)>0) m_val_tr_fast = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 10,0, 1, b)>0) m_val_tr_slow = (b[0]==EMPTY_VALUE)?0:b[0];
      }
