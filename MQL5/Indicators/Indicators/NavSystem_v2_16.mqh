...
//|                                             For Project Merkava  |
//|                                                   Version 2.16   |
//|        (Integration: Context v3.22 (v3.18 Logic) + Momentum)     |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.22
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.22) - EXACT v3.18 PARAMS
       PrintFormat("NavSystem: Loading Context (v3.22) from %s", ctx.path);
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.22 (Same as v3.18)
