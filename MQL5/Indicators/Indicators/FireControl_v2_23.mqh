//+------------------------------------------------------------------+
//|                                              FireControl_v2_23.mqh |
//|                                                      Jules Agent |
//|                                       Part of Merkava Tank Logic |
//|                                                    Version 2.23  |
//|                    (Stealth Engine & Registry v2.0)              |
//|                    (Fix: Force Empty Broker Comment)             |
//+------------------------------------------------------------------+
#ifndef FIRECONTROL_V2_23_MQH
#define FIRECONTROL_V2_23_MQH
...
#include "StealthRegistry.mqh" // v1.0
...
public:
   ...
   void ExecuteTrade(ENUM_ORDER_TYPE type, double vol, double price, double sl, double tp, bool deep_stealth, int layer_id = 0)
   {
       ...
       string internal_comment = m_comment_prefix;
       ...
       if(deep_stealth && m_registry != NULL) {
           ...
           // Append Layer ID to Internal Comment for audit
           if(layer_id > 0) internal_comment += "_L" + IntegerToString(layer_id);
       }

       // VERIFICATION: Print exactly what we are sending
       PrintFormat("STEALTH AUDIT: Sending Order. Magic=%I64u, Comment='%s' (Broker sees EMPTY), Internal='%s'", magic, comment, internal_comment);
       ...
       if(res) {
           if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE || m_trade.ResultRetcode() == TRADE_RETCODE_PLACED) {
               ...
               if(deep_stealth && m_registry != NULL && ticket > 0) {
                   // Register using the INTERNAL comment (which might have Layer info)
                   m_registry.RegisterTicket(ticket, magic, internal_comment);
               }
           }
       } else {
