...
//| v2.23: Updated for StealthRegistry v1.08 (Log Fix)               |
//+------------------------------------------------------------------+
class CFireControl
{
private:
   ...
   CStealthRegistry *m_registry; // Stealth Registry Pointer (v1.08 Class is still CStealthRegistry)
...
public:
   ...
   {
       ulong magic = m_magic;
       string comment = m_comment_prefix;

       // If Deep Stealth, override with Randoms
       if(deep_stealth && m_registry != NULL) {
           magic = m_registry.GetRandomMagic();
           comment = m_registry.GetRandomComment();
       ...
       if(res) {
           if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE || m_trade.ResultRetcode() == TRADE_RETCODE_PLACED) {
               ...
               if(deep_stealth && m_registry != NULL && ticket > 0) {
                   m_registry.RegisterTicket(ticket, magic, comment);
               }
           }
       }
