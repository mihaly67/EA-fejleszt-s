...
//+------------------------------------------------------------------+
//| Forensic Recorder - CSV Logging (v2.00)                          |
//| Core Improvement: Zero Latency, Unified Columns, Precision       |
//+------------------------------------------------------------------+
class CBlackBox
{
private:
...
public:
   CBlackBox()
   {
      ...
      m_headers = "Time,TickMSC,Phase,MimicMode,Verdict,Bid,Ask,Spread,BidVol,AskVol," +
                  ...
                  "Flow_MFI,Flow_ROC,Flow_Delta," + // UNIFIED v2.00
   ...
   bool Initialize(string symbol, string version)
   {
      ...
      m_is_active = true;
      Print("BlackBox v2.00 Recording to: ", m_filename);
      return true;
   }
   ...
   void RecordTick(
      long tick_time_msc, // Master Source of Truth
      ...
      double f_mfi, double f_roc, double f_delta, // v2.00 Unified Inputs
