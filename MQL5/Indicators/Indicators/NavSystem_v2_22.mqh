...
public:
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          ...
          if(CopyBuffer(m_handle_context, 10, 0, 1, b)>0) m_val_mic_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc
          ...
          if(CopyBuffer(m_handle_context, 11, 0, 1, b)>0) m_val_sec_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc
          ...
          if(CopyBuffer(m_handle_context, 12, 0, 1, b)>0) m_val_ter_p = (b[0]==EMPTY_VALUE)?0:b[0]; // Calc

          if(CopyBuffer(m_handle_context, 6, 0, 1, b)>0) m_val_tr_fast = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 7, 0, 1, b)>0) m_val_tr_medium = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 8, 0, 1, b)>0) m_val_tr_slow = (b[0]==EMPTY_VALUE)?0:b[0];
          if(CopyBuffer(m_handle_context, 9, 0, 1, b)>0) m_val_tr_super = (b[0]==EMPTY_VALUE)?0:b[0];
      }
