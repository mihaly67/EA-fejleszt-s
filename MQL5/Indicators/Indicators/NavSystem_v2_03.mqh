...
//+------------------------------------------------------------------+
//| Navigation System - Indicators & Sensors (v2.01)                 |
//| Improvement: Visualization & Aggressive Cleanup                  |
//+------------------------------------------------------------------+
class CNavSystem
{
private:
...
public:
   ...
   void Release()
   {
      // 1. Remove Indicators from Chart
      // We iterate through windows to find our indicators and remove them
      ...
      for(int w=windows-1; w>=0; w--) {
          ...
          for(int i=total-1; i>=0; i--) {
              ...
              // Remove our specific indicators
              if(StringFind(nlow, "hybrid") >= 0 ||
                 StringFind(nlow, "pulse") >= 0 ||
                 StringFind(nlow, "flow") >= 0)
              {
   ...
   {
       // Cleanup First (Prevent Stacking)
       Release();
       ...
       // 2. Create Handles (VISUALIZATION)
   ...
   // Attach to Chart (Explicit Call)
   void AttachToChart(long chart_id)
   {
       if(m_handle_hybrid_macd != INVALID_HANDLE)
           ChartIndicatorAdd(chart_id, 1, m_handle_hybrid_macd); // Subwindow 1

       if(m_handle_flow != INVALID_HANDLE)
           ChartIndicatorAdd(chart_id, 2, m_handle_flow); // Subwindow 2
   }
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      // Cold Start Fix: If MFI is stuck at 0 or 50 init, force calc on history first
      // But actually, we just need to ensure that if it's the very first tick, we treat it correctly.
      // The logic below updates the LAST bar.
      // If the EA starts mid-bar, m_rates[copied-1] is the current forming bar.

      // Update with Tick Precision
...
private:
   ...
   void CalcHybridPulse(int total)
   {
       ...
       // Fix: Ensure Point() is treated as double and scale is not truncating
       double pt = Point();
       if(pt == 0) pt = 0.00001; // Fallback safety

       double scale = p_df_auto ? pt : p_df_scale;

       // Force double precision multiplication
       m_val_dfcurve = (double)df_raw * (double)scale;
   }
