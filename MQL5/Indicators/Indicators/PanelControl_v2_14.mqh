...
//| v2.14: Full Feature Set (Trap, Directional, Burst/Grid)          |
//+------------------------------------------------------------------+
class CPanelControl
{
private:
   ...
   string ObjBG;
   string ObjStat;
   string ObjBtnFireTrap;   // V2.13 Logic (Both)
   string ObjBtnAttackBuy;  // Directional Buy
   string ObjBtnAttackSell; // Directional Sell
   string ObjBtnClear;
   string ObjBtnMode;
   string ObjBtnEntry;
   string ObjBtnAction;     // Scope (Grid vs Burst)
   string ObjLineSep;
...
public:
             ...
             double def_lot, double def_start, double def_step, int def_layers, double def_min_dist)
   {
      ...
      m_width = 180; // Standard Width (Not compact)
      m_height = 420; // Taller to fit all buttons comfortably
      ...
      ObjBtnFireTrap = m_prefix + "BtnTrap";
   ...
   void Create()
   {
       ...
       ObjectSetInteger(0, ObjStat, OBJPROP_FONTSIZE, 9);
       ...
       ObjectSetInteger(0, ObjLineSep, OBJPROP_XDISTANCE, x+10); ObjectSetInteger(0, ObjLineSep, OBJPROP_YDISTANCE, cy);
       ObjectSetInteger(0, ObjLineSep, OBJPROP_XSIZE, w-20); ObjectSetInteger(0, ObjLineSep, OBJPROP_YSIZE, 2);
       ...
       ObjectSetInteger(0, ObjBtnMode, OBJPROP_XSIZE, w-20); ObjectSetInteger(0, ObjBtnMode, OBJPROP_YSIZE, 25);
       ...
       ObjectSetInteger(0, ObjBtnEntry, OBJPROP_XSIZE, w-20); ObjectSetInteger(0, ObjBtnEntry, OBJPROP_YSIZE, 25);

       cy+=30;
       ObjectCreate(0, ObjBtnAction, OBJ_BUTTON, 0, 0, 0);
       ObjectSetInteger(0, ObjBtnAction, OBJPROP_XDISTANCE, x+10); ObjectSetInteger(0, ObjBtnAction, OBJPROP_YDISTANCE, cy);
       ObjectSetInteger(0, ObjBtnAction, OBJPROP_XSIZE, w-20); ObjectSetInteger(0, ObjBtnAction, OBJPROP_YSIZE, 25);

       // --- FIRE TRAP (ALL) BUTTON ---
       cy+=35;
       ObjectCreate(0, ObjBtnFireTrap, OBJ_BUTTON, 0, 0, 0);
       ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_XDISTANCE, x+10); ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_YDISTANCE, cy);
       ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_XSIZE, w-20); ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_YSIZE, 35);
       ObjectSetString(0, ObjBtnFireTrap, OBJPROP_TEXT, "FIRE TRAP (BOTH)");
       ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_BGCOLOR, clrRoyalBlue); ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_COLOR, clrWhite);
       ObjectSetInteger(0, ObjBtnFireTrap, OBJPROP_FONTSIZE, 8);
       ...
       ObjectSetInteger(0, ObjBtnAttackBuy, OBJPROP_XDISTANCE, x+10); ObjectSetInteger(0, ObjBtnAttackBuy, OBJPROP_YDISTANCE, cy);
       ...
       ObjectSetInteger(0, ObjBtnAttackSell, OBJPROP_XDISTANCE, x+10+btn_w+5); ObjectSetInteger(0, ObjBtnAttackSell, OBJPROP_YDISTANCE, cy);
   ...
   void UpdateButtons() {
        ...
        if(m_action_type == ACTION_SOLO) {
            ObjectSetString(0, ObjBtnAction, OBJPROP_TEXT, "SCOPE: BURST ONLY");
            ObjectSetInteger(0, ObjBtnAction, OBJPROP_BGCOLOR, clrPurple);
        } else {
            ObjectSetString(0, ObjBtnAction, OBJPROP_TEXT, "SCOPE: GRID + BURST");
   ...
   ENUM_PANEL_EVENT OnEvent(int id, long lparam, double dparam, string sparam)
   {
       ...
       {
          if(sparam == ObjBtnFireTrap)
          {
             m_attack_dir = ATTACK_BOTH; // Trap Mode (Both)
          ...
          else if(sparam == ObjBtnAttackBuy)
          {
