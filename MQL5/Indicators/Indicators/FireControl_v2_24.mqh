...
//|                                                    Version 2.24  |
//|                    (Stealth Engine & Registry v2.0)              |
//|                    (Fix: Forced Logging + Separate Tag Column)   |
//+------------------------------------------------------------------+
...
//| v2.22: Deep Stealth (Registry & Random Magic)                    |
//| v2.23: Stealth Fix (Force Empty Comment to Broker)               |
//| v2.24: Log Fix (Pass StrategyTag to Registry)                    |
//+------------------------------------------------------------------+
class CFireControl
{
private:
   ...
   CStealthRegistry *m_registry; // Stealth Registry Pointer
...
public:
   ...
   void FireGrid(double center_price, double lot_size, int layers, double spread_mult_start, double spread_mult_step, double min_spread_points, ENUM_FIRE_MODE fire_mode, ENUM_ENTRY_MODE entry_mode, ENUM_ATTACK_DIR attack_dir)
   {
      ...
      if (entry_mode == ENTRY_MARKET)
      {
          if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_BUY) {
              ...
              ExecuteTrade(ORDER_TYPE_BUY, lot_size, 0, 0, 0, deep_stealth, 1);
          }
          if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_SELL) {
              if(stealth_active && attack_dir == ATTACK_BOTH) m_stealth.ApplyHumanDelay();
              ExecuteTrade(ORDER_TYPE_SELL, lot_size, 0, 0, 0, deep_stealth, 1);
          }
      ...
      {
         int current_layer_id = pending_start_layer + (i-1); // Correct Layer ID for Audit
         ...
         if (fire_mode == FIRE_MODE_STOP)
         {
             ...
             if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_BUY) {
                 if(stealth_active) m_stealth.ApplyHumanDelay();
                 ExecuteTrade(ORDER_TYPE_BUY_STOP, lot_size, buy_price, 0, 0, deep_stealth, current_layer_id);
             }
             if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_SELL) {
                 if(stealth_active) m_stealth.ApplyHumanDelay();
                 ExecuteTrade(ORDER_TYPE_SELL_STOP, lot_size, sell_price, 0, 0, deep_stealth, current_layer_id);
             }
         }
         else
         {
             ...
             if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_BUY) {
                 if(stealth_active) m_stealth.ApplyHumanDelay();
                 ExecuteTrade(ORDER_TYPE_BUY_LIMIT, lot_size, buy_price, 0, 0, deep_stealth, current_layer_id);
             }
             if (attack_dir == ATTACK_BOTH || attack_dir == ATTACK_SELL) {
                 if(stealth_active) m_stealth.ApplyHumanDelay();
                 ExecuteTrade(ORDER_TYPE_SELL_LIMIT, lot_size, sell_price, 0, 0, deep_stealth, current_layer_id);
             }
         }
      }
   ...
   void ExecuteTrade(ENUM_ORDER_TYPE type, double vol, double price, double sl, double tp, bool deep_stealth, int layer_id = 0)
   {
       ulong magic = m_magic;
       string comment = ""; // FORCE EMPTY COMMENT FOR BROKER (Stealth Requirement)
       string strategy_tag = ""; // Internal Tag for Audit

       // If Deep Stealth, generate random magic
       if(deep_stealth && m_registry != NULL) {
           magic = m_registry.GetRandomMagic();
           m_trade.SetExpertMagicNumber(magic);

           // Construct Internal Tag (e.g. Merkava_L1)
           strategy_tag = m_comment_prefix;
           if(layer_id > 0) strategy_tag += "_L" + IntegerToString(layer_id);
       }

       // VERIFICATION: Print exactly what we are sending
       PrintFormat("STEALTH AUDIT: Sending Order. Magic=%I64u, Comment='%s' (Broker sees EMPTY), InternalTag='%s'", magic, comment, strategy_tag);

       bool res = false;
       if(type == ORDER_TYPE_BUY) res = m_trade.Buy(vol, m_symbol_name, price, sl, tp, comment);
       else if(type == ORDER_TYPE_SELL) res = m_trade.Sell(vol, m_symbol_name, price, sl, tp, comment);
       else if(type == ORDER_TYPE_BUY_STOP) res = m_trade.BuyStop(vol, price, m_symbol_name, sl, tp, ORDER_TIME_GTC, 0, comment);
       else if(type == ORDER_TYPE_SELL_STOP) res = m_trade.SellStop(vol, price, m_symbol_name, sl, tp, ORDER_TIME_GTC, 0, comment);
       else if(type == ORDER_TYPE_BUY_LIMIT) res = m_trade.BuyLimit(vol, price, m_symbol_name, sl, tp, ORDER_TIME_GTC, 0, comment);
       else if(type == ORDER_TYPE_SELL_LIMIT) res = m_trade.SellLimit(vol, price, m_symbol_name, sl, tp, ORDER_TIME_GTC, 0, comment);
       ...
       if(res) {
           if(m_trade.ResultRetcode() == TRADE_RETCODE_DONE || m_trade.ResultRetcode() == TRADE_RETCODE_PLACED) {
               ...
               if(deep_stealth && m_registry != NULL && ticket > 0) {
                   // Register using the STRATEGY TAG (passed as extra argument now)
                   // The 'comment' passed to RegisterTicket is what goes into the 'Comment' column (empty or random)
                   // The 'tag' goes into 'StrategyTag' column
                   m_registry.RegisterTicket(ticket, magic, comment, strategy_tag);
               }
           }
       } else {
           PrintFormat("STEALTH ERROR: Trade failed. Retcode=%d", m_trade.ResultRetcode());
       }
