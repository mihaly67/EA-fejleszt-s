...
//|                                             For Project Merkava  |
//|                                                   Version 2.14   |
//|        (Full Integration: Context v3.20 + Momentum + Pulse + Flow) |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.20
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.20 Support) - Linear Params
       PrintFormat("NavSystem: Loading Context (v3.20) from %s", ctx.path);
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.20 (Linear)
