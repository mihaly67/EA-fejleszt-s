...
//|                                             For Project Merkava  |
//|                                                   Version 2.19   |
//|        (Integration: Context v3.26 Flat/Reordered + Momentum)    |
//+------------------------------------------------------------------+
...
private:
   ...
   int      m_handle_flow;
   int      m_handle_context;     // Context v3.26
...
public:
   ...
   )
   {
       ...
       // 4. Context Indicator (v3.26 Reorder+Flat)
       PrintFormat("NavSystem: Loading Context (v3.26 Flat) from %s", ctx.path);
       m_handle_context = iCustom(symbol, period, ctx.path,
           ctx.show_pivots, ctx.show_trends, ctx.max_hist,
           false, 0, // Fibo Forced OFF in EA
   ...
   void Refresh(string symbol, MqlTick& latest_tick)
   {
      ...
      // Context - NEW MAPPING FOR v3.26
      if(m_handle_context != INVALID_HANDLE) {
          double b[1];
          // Buffer Indices from HybridContextIndicator_v3.26 (Reordered)
          // 0: MicroR (Data)
          // 1: MicroS (Data)
          // 2: SecR (Data)
          // 3: SecS (Data)
