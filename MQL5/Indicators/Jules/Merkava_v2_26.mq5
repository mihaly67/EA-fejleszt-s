...
#include "../Indicators/FireControl_v2_20.mqh"
#include "../Indicators/PanelControl_v2_20.mqh"
#include "../Indicators/PhysicsEngine.mqh"
#include "../Indicators/NavSystem_v2_17.mqh" // Direct v3.18 Support
...
// [Hybrid Context Settings - Reference v3.18]
// NOTE: We point DIRECTLY to the reference indicator v3.18
input string        InpContextPath       = "Jules\\HybridContextIndicator_v3.18";

input group         "=== Global Switches (v3.18 Ref) ==="
input bool          Ctx_ShowPivots       = true;
input bool          Ctx_ShowTrends       = true;
input int           Ctx_MaxHistory       = 50000;

input group         "=== Auto Fibo Settings (v3.18 Ref) ==="
// Forced OFF in logic usually, but exposed for completeness
input bool          Ctx_ShowFibo         = false;
input int           Ctx_FiboHist         = 0;

input group         "=== Micro ZigZag (Fast) Settings (v3.18 Ref) ==="
input bool          Ctx_Mic_Use          = true;
input int           Ctx_Mic_Depth        = 3;
input int           Ctx_Mic_Dev          = 5;
input int           Ctx_Mic_Back         = 3;
input ENUM_LINE_STYLE Ctx_Mic_Style      = STYLE_SOLID;
input int           Ctx_Mic_Width        = 1;
input color         Ctx_Mic_ColorR       = clrRed;
input color         Ctx_Mic_ColorS       = clrGreen;

input group         "=== Secondary ZigZag (Slow) Settings (v3.18 Ref) ==="
...
input group         "=== Tertiary ZigZag (Trend) Settings (v3.18 Ref) ==="
...
input group         "=== Trend Settings (v3.18 Ref) ==="
...
input group         "=== Momentum Settings (v2.82 Ref) ==="
...
int OnInit()
{
   ...
   // Prepare Context Params (v3.18 Reference Structure)
   ContextParams ctx;
   ...
   m_black_box.Initialize(_Symbol, "v2.26");
   ...
   Print("Merkava v2.26 Initialized (Context v3.18 Direct + Momentum v2.82).");
