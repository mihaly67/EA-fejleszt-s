//+------------------------------------------------------------------+
//|                            Hybrid_Microstructure_Monitor_v1.7.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|        Verzió: 1.7 (Pure Pressure + Tick Density Boost)          |
//|        Changes: v1.7 - Removed 'input group' for iCustom safety  |
//+------------------------------------------------------------------+
...
/*
   ===================================================================
   HYBRID MICROSTRUCTURE MONITOR v1.7
   ===================================================================
   1. LOGIKA (Pressure): (Alsó Kanóc - Felső Kanóc).
   2. VIZUALIZÁCIÓ (Hisztogram): ZÖLD (Bika), PIROS (Medve).
   3. TICK DENSITY: Sűrűség alapú boost.

   Safe for iCustom (No input groups).
*/
...
//--- Input Parameters
input int                InpLookbackVol        = 10;       // [Pressure] Lookback for Volume Avg
input double             InpPressureGain       = 100.0;    // [Pressure] Visual Gain (Amplitude)

input bool               InpUseTickAnalysis    = true;     // [Density] Enable Tick Density Boost
input double             InpTickHistoryDays    = 1.0;      // [Density] Tick History Limit (Days)
input double             InpHighVolThreshold   = 1.5;      // [Density] High Volume Threshold (Color Boost)
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Microstructure v1.7");
