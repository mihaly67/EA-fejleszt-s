...
//|                                             For Project Merkava  |
//|                                                   Version 2.18   |
//|                    (HybridContextIndicator v3.18 Integration Fix)  |
//+------------------------------------------------------------------+
...
input string        InpComment           = "Merkava_v2.18";
...
// Fibo Forced OFF internally, but input remains for compatibility with struct
...
int OnInit()
{
   ...
   ctx_params.show_fibo = InpShowFibo; ctx_params.fibo_hist = InpFiboMicroHistory; // Passed but NavSystem forces OFF
   // Micro
   ...
   m_black_box.Initialize(_Symbol, "v2.18"); // Update Log Version
   ...
   Print("Merkava v2.18 Initialized (Context Integr. + DEBUG + Buffer Fix).");
...
void OnTick()
{
   ...
   // --- DEBUG BUFFER VALUES ---
   g_debug_counter++;
   if(g_debug_counter % 10 == 0) { // Log every 10 ticks to avoid spam
       PrintFormat("DEBUG TICK: Micro R=%.5f, S=%.5f | Sec R=%.5f, S=%.5f | Ter R=%.5f, S=%.5f",
                   m_nav_system.GetMicR(), m_nav_system.GetMicS(),
                   m_nav_system.GetSecR(), m_nav_system.GetSecS(),
                   m_nav_system.GetTerR(), m_nav_system.GetTerS());
       // Check for bad values
       if(m_nav_system.GetMicR() == 0.0) Print("⚠️ WARNING: Micro R is 0.0 (Possible EMPTY_VALUE)");
   }
   // ---------------------------
