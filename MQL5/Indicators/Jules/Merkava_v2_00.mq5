...
// Library Organization (Relative to MQL5/Indicators/Jules/)
#include "../Indicators/FireControl.mqh"
#include "../Indicators/PhysicsEngine.mqh"
#include "../Indicators/NavSystem_v2_00.mqh"
#include "../Indicators/BlackBox_v2_00.mqh"
...
// [Hybrid & Flow Settings for NavSystem]
...
int OnInit()
{
   ...
   // Init NavSystem (Internal Math Only - No visual chart indicators created by EA logic needed for math)
   // We pass paths just in case we want to re-enable visuals later, but logic is internal now.
   m_nav_system.Initialize(
       _Symbol, _Period,
   ...
   // Init BlackBox v2.00
   m_black_box.Initialize(_Symbol, "v2.00");

   // Init Forensic History
   ...
   Print("Merkava v2.00 Initialized (Zero Latency Mode).");
...
//+------------------------------------------------------------------+
//| Main Tick Loop (Zero Latency)                                    |
//+------------------------------------------------------------------+
void OnTick()
{
   // 1. Get Absolute Truth (Tick)
   MqlTick tick;
   ...
   // 3. Update NavSystem (Internal Math)
   m_nav_system.Refresh(_Symbol, tick);

   // 4. Update Financials & Forensics
   CheckForNewDeals();

   // 5. Log Data (Zero Latency)
...
void CreatePanel() {
   ...
   ObjectSetString(0, ObjStat, OBJPROP_TEXT, "MERKAVA v2.00");
