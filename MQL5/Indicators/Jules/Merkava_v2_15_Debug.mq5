...
#include "../Indicators/Types_v2_15.mqh" // Types first
#include <Trade\Trade.mqh>
#include <Trade\SymbolInfo.mqh>
#include <Trade\PositionInfo.mqh>
#include <AccountInfo.mqh>

// Library Organization (v2.15)
#include "../Indicators/FireControl_v2_15.mqh"
#include "../Indicators/PanelControl_v2_15.mqh"
#include "../Indicators/PhysicsEngine.mqh"
#include "../Indicators/NavSystem_v2_06.mqh"
#include "../Indicators/BlackBox_v2_05.mqh"
#include "../Indicators/ProfitManagement_v2_15.mqh" // Added ProfitManager
...
input string        InpComment           = "Merkava_DEBUG";     // Default Empty
...
int OnInit()
{
   ...
   // --- DEBUG LOGGING ---
   Print("🔍 MERKAVA DEBUG INIT START");
   Print("--------------------------------------------------");
   PrintFormat("🔢 ACCOUNT: Leverage=%d | Currency=%s", AccountInfoInteger(ACCOUNT_LEVERAGE), AccountInfoString(ACCOUNT_CURRENCY));
   PrintFormat("🔢 MARGIN: Mode=%d (0=Forex, 1=CFD, 2=Futures) | Limit=%.1f%%", AccountInfoInteger(ACCOUNT_MARGIN_MODE), InpMaxMarginPercent);
   PrintFormat("🔢 SYMBOL: MinVol=%.2f | MaxVol=%.2f | Step=%.2f", SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN), SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MAX), SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP));

   double initial_margin;
   if(OrderCalcMargin(ORDER_TYPE_BUY, _Symbol, 1.0, SymbolInfoDouble(_Symbol, SYMBOL_ASK), initial_margin)) {
       PrintFormat("💰 MARGIN CHECK: 1.0 Lot Buy requires %.2f %s", initial_margin, AccountInfoString(ACCOUNT_CURRENCY));
   } else {
       Print("❌ MARGIN CHECK FAILED: Could not calculate margin for 1.0 Lot.");
   }
   Print("--------------------------------------------------");
   // --- END DEBUG LOGGING ---
   ...
   m_black_box.Initialize(_Symbol, "v2.15_DEBUG"); // Update Log Version
   ...
   Print("Merkava v2.15 DEBUG Initialized (Logging Critical Stats).");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if (event != EVENT_NONE)
   {
       ...
       {
           // TRAP (BOTH)
           PrintFormat("🔥 FIRE BOTH Triggered. Lot=%.2f", lot); // Debug Log
       ...
       {
           // BUY ONLY
           PrintFormat("🔥 FIRE BUY Triggered. Lot=%.2f", lot); // Debug Log
       ...
       {
           // SELL ONLY
           PrintFormat("🔥 FIRE SELL Triggered. Lot=%.2f", lot); // Debug Log
