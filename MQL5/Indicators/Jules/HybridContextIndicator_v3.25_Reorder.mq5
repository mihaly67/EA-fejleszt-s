//+------------------------------------------------------------------+
//|                          HybridContextIndicator_v3.25_Reorder.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 3.25 (Buffer Reordering Fix - FULL IMPLEMENTATION)   |
//|      Logic: All DATA buffers (0-7) first, CALC buffers (8-10) last|
//+------------------------------------------------------------------+
...
//--- PLOT 8: Trend EMA Slow (Buffer 7)
...
//--- Input Parameters (Standard v3.18 Structure)
...
int OnInit()
{
   ...
   // --- CALCULATIONS LAST (8-10) ---
   SetIndexBuffer(8, MicroP, INDICATOR_CALCULATIONS);
   SetIndexBuffer(9, SecP, INDICATOR_CALCULATIONS);
   SetIndexBuffer(10, TerP, INDICATOR_CALCULATIONS);

   IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Context v3.25 (Reorder)");
                ...
                const int &spread[])
{
   ...
   if(InpShowTrends)
   {
      if(ema_fast_handle != INVALID_HANDLE) CopyBuffer(ema_fast_handle, 0, 0, rates_total, TrendFast); // Buffer 6
      if(ema_slow_handle != INVALID_HANDLE) CopyBuffer(ema_slow_handle, 0, 0, rates_total, TrendSlow); // Buffer 7
   }
