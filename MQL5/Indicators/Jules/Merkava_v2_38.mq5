...
//|                                             For Project Merkava  |
//|                                                   Version 2.37   |
//|                    (Deep Stealth Deep Randomization & CSV Fix)   |
//+------------------------------------------------------------------+
...
#include "../Indicators/FireControl_v2_23.mqh" // Deep Stealth v2.23 (Force Empty Comment)
#include "../Indicators/StealthEngine.mqh"     // Stealth Engine v1.0
#include "../Indicators/StealthRegistry.mqh"   // Deep Stealth Registry v1.05 (Deep Random & LCG) - HOTFIX
...
CStealthRegistry m_registry; // Deep Stealth Registry Instance
...
input string        InpComment           = "Merkava_v2.37"; // Fallback only
...
// [Hybrid Context Settings v3.27 - FLAT STRUCTURE - STYLE FIX]
...
int OnInit()
{
   ...
   if(DeepStealth_Enabled) {
       ...
       reg_ptr = &m_registry;
       Print("🕵️ Deep Stealth Registry ACTIVATED. Random Magic Numbers Enabled (v1.05 LCG).");

       // Explicit CSV Test on Init
       Print("🔬 Registry Test: Trying to write to log now...");
       // This will force creation if missing or log an error if permission denied
       m_registry.LogAudit("INIT_TEST", 0, 0, "Merkava v2.37 Started");
   }
   ...
   m_black_box.Initialize(_Symbol, "v2.37");

   // --- DYNAMIC VERSION LABEL (Fixed) ---
   string version_str = "MERKAVA v2.38 (Stealth Fix)";
   ...
   Print("Merkava v2.38 Initialized (Stealth Fix).");
