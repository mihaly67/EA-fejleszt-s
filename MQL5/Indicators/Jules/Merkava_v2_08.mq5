...
// Library Organization (v2.08)
#include "../Indicators/FireControl_v2_08.mqh" // v2.08: Barbed Wire Trap Logic
...
input double        InpMinSpreadPoints   = 60.0; // v2.08: Adaptive Spread Minimum
...
input ulong         InpMagicNumber       = 999008; // Updated Magic for v2.08
input string        InpComment           = "Merkava_Trap";
...
int OnInit()
{
   ...
   // v2.08: Initialize FireControl with pointers
   ...
   m_black_box.Initialize(_Symbol, "v2.08");
   ...
   Print("Merkava v2.08 Initialized (Barbed Wire Trap - Stop Orders).");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == ObjBtnFire)
      {
         ...
         // v2.08: Pass InpMinSpreadPoints for Adaptive Logic
         m_fire_control.FireBurst(center, g_user_lot_size, InpLayers, InpSpreadMultStart, InpSpreadMultStep, InpMinSpreadPoints);
         g_last_action = "BURST_FIRED";
         g_decision_log += "Trap Fired L" + IntegerToString(InpLayers) + ";";
...
void CreatePanel() {
   ...
   ObjectSetString(0, ObjStat, OBJPROP_TEXT, "MERKAVA v2.08");
