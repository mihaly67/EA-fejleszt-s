//+------------------------------------------------------------------+
//|                         Jules_Hybrid_Momentum_Pulse_v1.05.mq5    |
//|                                     Jules Agent & User           |
//|                 Hybrid Momentum Pulse (Scaled: Divisor 7.0)      |
//+------------------------------------------------------------------+
#property copyright "Jules Agent (Hybrid)"
#property link      "https://mql5.com"
#property version   "1.05"
#property description "Hybrid Momentum Pulse: Scaled for Precision (Divisor 7.0)"
...
input double         InpDivisor           = 7.0;         // [Hybrid] Global Divisor (Precision)
...
int OnInit()
  {
   ...
   {
       Print("Hybrid Pulse v1.05: Failed to create handles!");
       return INIT_FAILED;
   }
   ...
   SetIndexBuffer(5, BufferBBMA, INDICATOR_CALCULATIONS);
   SetIndexBuffer(6, BufferBBDEV, INDICATOR_CALCULATIONS);
   SetIndexBuffer(7, BufferKMA, INDICATOR_CALCULATIONS);
   SetIndexBuffer(8, BufferKATR, INDICATOR_CALCULATIONS);

   SetIndexBuffer(9, BufferDFRawH, INDICATOR_CALCULATIONS);
   SetIndexBuffer(10, BufferDFRawL, INDICATOR_CALCULATIONS);

   // 3. Properties
   IndicatorSetString(INDICATOR_SHORTNAME, "Jules_Hybrid_Momentum_Pulse_v1.05");
   IndicatorSetInteger(INDICATOR_DIGITS, 5); // Force 5 decimals

   PlotIndexSetInteger(1, PLOT_SHIFT, InpDFShift); // Shift Plot 1 (DF Curve)
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   // We only need these if we calculate Squeeze Dots (future use)
   ...
   // Scaling Logic (Divisor Application)
   double global_divisor = (InpDivisor != 0) ? InpDivisor : 1.0;

   for(int i = limit; i >= 0 && !IsStopped(); i--)
   {
       // --- A. MACD HISTOGRAM LOGIC ---
       double raw_macd = (BufferFMA[i] - BufferSMA[i]);
       // Apply Scale AND Divisor
       double final_macd = (raw_macd * InpMACDScale) / global_divisor;
       BufferMACDHist[i] = final_macd;

       // Color Logic
       double prev_macd = (i < rates_total-1) ? BufferMACDHist[i+1] : 0;
       if (final_macd > prev_macd) {
       ...
       // Apply Scaling AND Divisor
       double df_scale_factor = InpUseAutoScaling ? Point() : InpDFScale;
       double final_df = (df_raw * df_scale_factor) / global_divisor;

       BufferDFCurve[i] = final_df;
   }
