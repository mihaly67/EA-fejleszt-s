...
// Library Organization (v2.10)
#include "../Indicators/FireControl_v2_10.mqh" // v2.10: Adaptive Spread Logic
...
input double        InpMinSpreadPoints   = 60.0; // v2.10: Adaptive Minimum Spread
...
input ulong         InpMagicNumber       = 999010; // Updated Magic v2.10
input string        InpComment           = "Merkava_v2.10";
...
int OnInit()
{
   ...
   // v2.10: Initialize FireControl with pointers
   ...
   m_black_box.Initialize(_Symbol, "v2.10"); // Update Log Version
   ...
   Print("Merkava v2.10 Initialized (Adaptive Spread + Full GUI Control).");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      if(sparam == ObjBtnFire)
      {
         ...
         // v2.10: Use Dynamic Global Variables from Panel
         m_fire_control.FireBurst(center, g_user_lot_size, g_user_layers, g_user_mult_start, g_user_mult_step, g_user_min_dist);

         g_last_action = "BURST_FIRED";
         g_decision_log += "Burst Fired L" + IntegerToString(g_user_layers) + " (Adaptive);";
   ...
   else if(id == CHARTEVENT_OBJECT_ENDEDIT)
   {
      ...
      if(sparam == ObjEditLot) {
           ...
           if(val > 0) g_user_mult_step = val; // Step can be small, but usually positive
      }
...
//+------------------------------------------------------------------+
//| Panel Construction (v2.10 Expanded)                              |
//+------------------------------------------------------------------+
void CreatePanel() {
   int x=InpX, y=InpY;
   int w=160;
   int h=320; // Expanded height
   ...
   ObjectSetString(0, ObjStat, OBJPROP_TEXT, "MERKAVA v2.10");
