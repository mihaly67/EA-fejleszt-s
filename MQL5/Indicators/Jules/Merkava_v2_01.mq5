...
// Library Organization (v2.01)
#include "../Indicators/FireControl.mqh"
#include "../Indicators/PhysicsEngine.mqh"
#include "../Indicators/NavSystem_v2_01.mqh"
#include "../Indicators/BlackBox_v2_01.mqh"
...
int OnInit()
{
   // 1. Aggressive Cleanup First (Prevent Ghosting)
   CleanupChart();
   m_nav_system.Release(); // Ensure old indicators are gone
   ...
   // Init NavSystem (v2.01 - With Visualization)
   // We pass params for BOTH internal math AND to create the visual indicators
   bool init_ok = m_nav_system.Initialize(
       _Symbol, _Period,
       InpIndPath + "Jules_Hybrid_Momentum_Pulse_v1.04",
   ...
   if(init_ok) {
       // Explicitly attach visual indicators to the chart windows
       m_nav_system.AttachToChart(0);
   }

   // Init BlackBox v2.01
   m_black_box.Initialize(_Symbol, "v2.01");
   ...
   Print("Merkava v2.01 Initialized.");
...
void OnDeinit(const int reason)
{
   // Aggressive Shutdown
   DestroyPanel();
   CleanupChart(); // Deletes all objects

   m_nav_system.Release(); // Deletes indicators from windows
   m_black_box.CloseLog();

   if(g_book_subscribed) MarketBookRelease(_Symbol);
   Print("Merkava v2.01 Deinitialized and Cleaned up.");
}
...
void CleanupChart()
{
   ...
   // Safety: If there are orphan objects from previous crashes that didn't clean up,
   // we might want to target them if they share a common naming pattern.
   // Assuming standard EA prefix "Merkava" or similar if known.
   // For now, we stick to the defined Prefix "MerkavaV2_" to avoid deleting user drawings.
...
void OnTick()
{
   ...
   // Internal Math Update (Zero Latency)
...
void CreatePanel() {
   ...
   ObjectSetString(0, ObjStat, OBJPROP_TEXT, "MERKAVA v2.01");
