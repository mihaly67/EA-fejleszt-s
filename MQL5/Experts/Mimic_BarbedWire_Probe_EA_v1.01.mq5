//+------------------------------------------------------------------+
//|                                  Mimic_BarbedWire_Probe_EA_v1.01 |
//|                                                      Jules Agent |
//|                       Focused Strategy: Barbed Wire (Szögesdrót) |
//|                       Mode: PROBE & STRESS TEST (v1.01)          |
//+------------------------------------------------------------------+
...
input color         InpBgColor           = clrDarkSlateGray; // [UI] BG Color (Restored v2.11)
...
string            g_decision_log = ""; // New: Detailed Decision Logic
...
double            g_last_layer_center = 0.0; // Track the center price of the last layer
...
void ExpandBarbedWire(double center_price, string trigger_source);
...
int OnInit()
  {
   ...
   // Init Log (v2.11 Format without DOM + DIAGNOSTICS)
   string time_str = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
   StringReplace(time_str, ":", ""); StringReplace(time_str, " ", "_");
   string filename = "Mimic_Probe_WIRE_" + _Symbol + "_v1.01_" + time_str + ".csv";
   g_log_handle = FileOpen(filename, FILE_WRITE|FILE_TXT|FILE_ANSI);
   if(g_log_handle != INVALID_HANDLE) {
      // Header: LastEvent extended with Decision_Log
      string header = "Time,TickMS,Phase,LotSize,TargetSpread,Bid,Ask,Spread,Velocity,Acceleration,Hybrid_MACD,Hybrid_Color,Hybrid_DFCurve,Flow_MFI,Flow_DUp,Flow_DDown,Mom_Hist,Pivot_PP,Pivot_R1,Pivot_S1,Floating_PL,Realized_PL,Session_PL,Action,PosCount,ActiveSL,ActiveTP,LastEvent,Decision_Log\r\n";
   ...
   Print("Mimic Barbed Wire Probe EA v1.01 Initialized.");
...
void OnTick()
  {
   ...
   g_tick_event_buffer = "";
   g_decision_log = ""; // Clear diagnostics
  }
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
     {
      ...
      if (g_active && entry == DEAL_ENTRY_IN)
      {
          ...
          // Debugging info
          string info = StringFormat("Filled @ %.5f (PrevCenter: %.5f)", price, g_last_layer_center);
          
          if (StringFind(comment, "Wire") >= 0 || StringFind(comment, "Breach") >= 0) {
              g_tick_event_buffer += "LAYER_FILLED;";
              // Add to decision log
              g_decision_log += "BREACH:" + info + ";";
              ExpandBarbedWire(price, "BREACH");
          }
      }
     }
  }
...
void StartBarbedWire()
  {
   ...
   double center = (ask+bid)/2.0;
   
   g_last_layer_center = center;
   ...
   g_decision_log += "START_CENTER:" + DoubleToString(center, 5) + ";";
   
   // 2. First Layer
   ExpandBarbedWire(center, "START");
...
void ExpandBarbedWire(double center_price, string trigger_source)
  {
   ...
   // Diagnostic Logging
   double dist_from_prev = MathAbs(center_price - g_last_layer_center);
   string decision = StringFormat("EXPAND: Src=%s | Center=%.5f | Prev=%.5f | Delta=%.5f | Spread=%.5f | CalcDist=%.5f", 
       trigger_source, center_price, g_last_layer_center, dist_from_prev, spread, dist);
   g_decision_log += decision + ";";
   
   // Update State
   g_last_layer_center = center_price;
...
void WriteLog()
  {
   ...
   double spread = m_symbol.Ask() - m_symbol.Bid();
   double target_dist = spread * InpSpreadMult;
   
   // Form CSV Row (With Diagnostics)
   string row = StringFormat("%s,%s,%s,%.2f,%.5f,%.5f,%.5f,%.1f,%.5f,%.5f,%.5f,%.0f,%.5f,%.2f,%.2f,%.2f,%.5f,%.5f,%.5f,%.5f,%.2f,%.2f,%.2f,%s,%d,%.5f,%.5f,%s,%s",
       TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), 
       IntegerToString(GetTickCount()%1000), 
       g_current_phase,
       g_user_lot_size, target_dist,
...
void CreatePanel()
  {
   ...
   ObjectSetInteger(0, ObjBG, OBJPROP_BGCOLOR, InpBgColor); // DarkSlateGray
   ...
   ObjectSetString(0, ObjStat, OBJPROP_TEXT, "READY v1.01");
   cy += 25;
