...
//--- Configuration File Name (For Color settings only, not Window State)
...
public:
   ...
   // Override Standard Library persistence to prevent layout jumping
   virtual void      IniFileSave(void) { /* Disabled to enforce fixed layout */ }
   virtual void      IniFileLoad(void) { /* Disabled to enforce fixed layout */ }
...
bool CChartDesignerPanel::Create(const long chart,const string name,const int subwin,const int x1,const int y1,const int x2,const int y2)
  {
   // Force CAppDialog creation without auto-loading state
...
int OnInit()
  {
   ...
   // Force larger size for Pro Palette
   int width = 280;
   int height = 550;
...
void OnChartEvent(const int id,
                  ...
                  const string &sparam)
  {
   ...
   if(id == CHARTEVENT_CHART_CHANGE) {
      ...
      // Move if necessary
      if(ExtDialog.Left() != x1) {
         ExtDialog.Move(x1, 40);
      }
   }
