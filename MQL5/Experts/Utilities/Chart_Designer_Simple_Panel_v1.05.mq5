//+------------------------------------------------------------------+
//|                             Chart_Designer_Simple_Panel_v1.05.mq5 |
//|                             Copyright 2026, Jules AI Agent       |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, Jules AI Agent"
#property version   "1.05"
#property strict
#property description "Simple Chart Designer Panel - v1.05 Final Robust Version"
...
{
   Print("Initializing SimplePanel v1.05...");
...
void CSimplePanel::ApplyColor(color c)
{
   ...
   else if(m_active_target == "GRID")      res = ChartSetInteger(chart, CHART_COLOR_GRID, c);
   else if(m_active_target == "TEXT")      res = ChartSetInteger(chart, CHART_COLOR_FOREGROUND, c);
   else if(m_active_target == "BULL_BODY") res = ChartSetInteger(chart, CHART_COLOR_CANDLE_BULL, c);
   else if(m_active_target == "BEAR_BODY") res = ChartSetInteger(chart, CHART_COLOR_CANDLE_BEAR, c);
   else if(m_active_target == "BULL_WICK") res = ChartSetInteger(chart, CHART_COLOR_CHART_UP, c);
   else if(m_active_target == "BEAR_WICK") res = ChartSetInteger(chart, CHART_COLOR_CHART_DOWN, c);

   if(!res) {
      Print("Error: ApplyColor failed! Error code: ", GetLastError());
   } else {
      ChartRedraw(chart);
      Print("ApplyColor [", m_active_target, "] -> ", c, " Success. Color applied.");
      UpdateStatus("Set " + m_active_target + " to " + (string)c);
   }
}

//+------------------------------------------------------------------+
//| OnEvent (Verified Hybrid Logic)                                  |
//+------------------------------------------------------------------+
bool CSimplePanel::OnEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      // Optimization: Check prefix "Pal_" first to avoid unnecessary loops
      else if(StringFind(sparam, "Pal_") >= 0) {
         // Loop to find exact match
         for(int i=0; i<100; i++) {
            if(sparam == m_palette[i].Name()) {
               Print("Palette Click Detected: Index ", i);
...
int OnInit()
{
   ...
   Print("OnInit: Starting Chart_Designer_Simple_Panel v1.05...");

   // Create the panel
   if(!ExtDialog.Create(0, "SimplePanel_v105", 0, InpPanelX, InpPanelY, InpPanelX + width, InpPanelY + height)) {
