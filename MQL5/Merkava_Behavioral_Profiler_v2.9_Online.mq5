//+------------------------------------------------------------------+
//|                             Merkava_Behavioral_Profiler_v2.9_Online.mq5 |
//|                                    Copyright 2026, Jules (Mimic) |
//|                                             For Project Merkava  |
//|                                                   Version 2.9 (Added DOM MarketBookGet + Socket payload support)    |
...
int OnInit()
{
   ...
   // --- EXPORT 1-DAY HISTORICAL BARS FOR PYTHON HUD (v1.55) ---
   if(InpEnablePythonBridge) {
       ...
       if(copied > 0) {
           int hist_handle = FileOpen("history_init.csv", FILE_WRITE|FILE_CSV|FILE_ANSI, ",");
           if(hist_handle != INVALID_HANDLE) {
               FileWrite(hist_handle, "time", "open", "high", "low", "close"); // Header
               for(int i = copied - 1; i >= 0; i--) {
                   string time_str = IntegerToString(rates[i].time);
                   string open_str = DoubleToString(rates[i].open, _Digits);
                   string high_str = DoubleToString(rates[i].high, _Digits);
                   string low_str  = DoubleToString(rates[i].low, _Digits);
                   string close_str= DoubleToString(rates[i].close, _Digits);
                   FileWrite(hist_handle, time_str, open_str, high_str, low_str, close_str);
               }
               FileClose(hist_handle);
               Print("Historical data (100 bars) exported successfully to history_init.csv for HUD.");
           } else {
               Print("Failed to open history_init.csv for writing!");
           }
       } else {
...
void SendMacroToPython(double current_price) {
    ...
    if(!g_zigzag_init_sent) {
        int init_bars = 100; // 24 hours of M1 data for stable ZigZag init
