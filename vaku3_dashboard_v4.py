...
class VakuDashboard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("VAKU 3.0 - Valós Idejű Advisory Műszerfal V4")
        ...
        self.playback_speed_multiplier = 1.0 # Alapból valós idő (1x)
        self.is_paused = False

        self.init_ui()

        # GUI frissítési frekvencia (mindig gyors, de az adatot a sebesség határozza meg)
        self.timer = QTimer()
        self.timer.timeout.connect(self.update_dashboard)
        self.timer.start(16) # ~60 FPS GUI
    ...
    def init_ui(self):
        ...
        # ==========================================
        # 0. VEZÉRLŐ PANEL (Lejátszás, Sebesség, Eltolás)
        # ==========================================
        ...
        btn_pause.setStyleSheet("background-color: #333; padding: 5px;")
        ...
        # ==========================================
        # 2. PYQTGRAPH DIAGRAMOK (X-Eltolással és Interakciókkal)
        # ==========================================
        ...
        # Egér interakciók engedélyezése, de AutoRange kikapcsolása az X tengelyen,
        # hogy mi mondjuk meg, hol van a jelen.
        ...
        self.p2.setMouseEnabled(x=True, y=False) # Y marad fix 0-100
    ...
    def change_speed(self, index):
        ...
        elif index == 4: self.playback_speed_multiplier = 1000.0 # Max
    ...
    def update_dashboard(self):
        ...
        # Időszinkronizálás a CSV alapján (1x sebességhez)
        current_real_time = time.time()

        # Feldolgozunk addig a tickig, amíg az idő el nem éri a célidőt
        ticks_processed_this_frame = 0
        max_ticks_per_frame = 100 # Ha "MAX" a sebesség, mennyit dobjon be egy renderbe

        if self.playback_speed_multiplier > 50:
            target_ticks = max_ticks_per_frame
        else:
            # Csak egy tick-et olvasunk be egyelőre, de elvileg a CSV TimeMSC különbségét kéne nézni
            target_ticks = int(self.playback_speed_multiplier)
            if target_ticks < 1: target_ticks = 1

        while ticks_processed_this_frame < target_ticks:
        ...
        # Úgy állítjuk be a ViewBoxot, hogy a jelen (utolsó tick) a képernyő 75%-ánál legyen
        latest_time = x_draw[-1]
        earliest_time = x_draw[0]
        time_span = latest_time - earliest_time

        # Kinyújtjuk a látóteret jobbra 25%-kal
        x_min = earliest_time
        x_max = latest_time + (time_span * 0.25)

        # AutoRange kikapcsolva, mi mondjuk meg hol tart a kamera
