    ...
    if target_dir and os.path.exists(target_dir):
        ...
        if check_path and os.path.exists(check_path):
            ...
            if os.path.getsize(check_path) > 1024:
                ...
                # Note: Don't rmtree Knowledge_Base for a single file, only if it's a dedicated dir.
                # For THIEFS and COLUMBO, they share Knowledge_Base.
                # We should only remove the specific file.
                if check_file:
                    try: os.remove(check_path)
                    except: pass
        else:
            print(f"   ⚠️ {check_file} missing in {target_dir}. Downloading...")
...
def main():
    ...
    if not new_ignores.issubset(ignores):
        with open(".gitignore", "a") as f:
            f.write("\n# Auto-generated by restore_env_TC.py\n")
