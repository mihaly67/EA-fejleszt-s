import time
import pandas as pd

class MarketSimulator:
    def __init__(self, m1_df, m5_df, m15_df):
        self.m1_df = m1_df
        self.m5_df = m5_df
        self.m15_df = m15_df
        # Create an iterator for the fastest timeframe (M1)
        self.m1_iterator = self.m1_df.iterrows()

    def fetch_next_tick(self):
        try:
            m1_time, m1_row = next(self.m1_iterator)

            # Floor M1 time to the nearest 5 minutes
            m5_time_floored = m1_time.floor('5min')
            m5_row = None
            if m5_time_floored in self.m5_df.index:
                m5_row = self.m5_df.loc[m5_time_floored]

            # Floor M1 time to the nearest 15 minutes
            m15_time_floored = m1_time.floor('15min')
            m15_row = None
            if m15_time_floored in self.m15_df.index:
                m15_row = self.m15_df.loc[m15_time_floored]

            return {
                'm1_time': m1_time,
                'm1_data': m1_row,
                'm5_time': m5_time_floored,
                'm5_data': m5_row,
                'm15_time': m15_time_floored,
                'm15_data': m15_row
            }
        except StopIteration:
            return None

if __name__ == '__main__':
    from data_pipeline import load_and_resample
    csv_file = '/home/misi/Merkava_ML_Ops/data/Merkava_XAUUSD_v1.10_20260408_025931.csv'
    m1, m5, m15 = load_and_resample(csv_file)
    sim = MarketSimulator(m1, m5, m15)

    print('Testing simulator for 10 ticks...')
    for i in range(10):
        tick = sim.fetch_next_tick()
        if tick:
            print(f"Tick {i+1}: M1={tick['m1_time']} | M5={tick['m5_time']} | M15={tick['m15_time']}")
        time.sleep(0.05)
