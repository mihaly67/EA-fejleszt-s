...
void CreatePanelAndControls()
  {
   ...
   ObjectSetString(0, panel_title_name, OBJPROP_TEXT, "WPR Control Trader v3.10");
...
int CalculateSignalStrength(ENUM_ORDER_TYPE signal_type)
{
    ...
    else { max_score -= 3; Print("Signal Strength: Not enough tick data for Look Back analysis."); }
    ...
    } else { max_score -= 2; Print("Signal Strength: Not enough data for Correlation analysis.");}
...
void ManageOpenPosition(const double prev_wpr_value, const double current_wpr_value)
  {
   ...
   switch(InpProfitMode)
   {
       ...
       case MODE_PSAR:
           if(CheckPointer(g_trailings) == POINTER_VALID)
           {
...
void OnDeinit(const int reason)
  {
   ...
   if(CheckPointer(g_trailings) == POINTER_VALID) delete g_trailings;
