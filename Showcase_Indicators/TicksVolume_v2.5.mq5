...
#property description "Includes Intensive Dynamic Scaling to fill available space."
...
#property indicator_color4  C'60,60,60'
...
input double InpTickScale       = 5.0; // Tick Bar Scaling Factor (vs Pips)
input double InpVolumeBoost     = 1.0; // Intensive Volume Boost (Overall Multiplier)
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "TicksVolume v2.5");
   ...
   for(int i = limit; i >= 0; i--)
     {
      ...
      // Apply Scaling to Tick Counts AND Global Boost
      UpBuffer[i] = (p_up / _Point) * InpVolumeBoost;
      DnBuffer[i] = -1.0 * (p_dn / _Point) * InpVolumeBoost;

      // Ticks Scale + Boost
      UpTick[i]   = t_up * InpTickScale * InpVolumeBoost;
      DnTick[i]   = -1.0 * t_dn * InpTickScale * InpVolumeBoost;
     }
