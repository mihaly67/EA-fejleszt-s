//+------------------------------------------------------------------+
//|                                 HybridMomentumIndicator_v1.8.mq5 |
//|                     Copyright 2024, Gemini & User Collaboration |
//|      Verzió: 1.8 (Stabilized DEMA with Gain Control & Phase)      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Generalized DEMA calculation (Gain Control)                      |
//+------------------------------------------------------------------+
void CalculateGeneralizedDEMA(const int rates_total, const int start_pos, const int period,
                              const double gain, const double &src[], double &dema_buffer[])
{
    if(period <= 0) return;
    ...
    // Standard DEMA: 2*EMA1 - 1*EMA2  (Gain = 2.0)
    // Stabilized: Gain*EMA1 - (Gain-1)*EMA2
    ...
    for(int i = 0; i < rates_total; i++)
    {
       ...
       dema_buffer[i] = gain * ema1 - (gain - 1.0) * ema2;
    }
}
...
int OnInit()
{
    ...
    IndicatorSetString(INDICATOR_SHORTNAME, "Hybrid Momentum v1.8");
                ...
                const int &spread[])
{
    ...
    double price_data[];
    if(start == min_bars_required) ArrayResize(price_data, rates_total);
    else ArrayResize(price_data, rates_total);
    ...
    // 1. Calculate Generalized DEMA (with Gain control)
    CalculateGeneralizedDEMA(rates_total, 0, InpFastDEMAPeriod, InpDemaGain, price_data, fast_dema_buffer);
    CalculateGeneralizedDEMA(rates_total, 0, InpSlowDEMAPeriod, InpDemaGain, price_data, slow_dema_buffer);
    ...
    // 2. Signal Line (Standard DEMA logic on MACD, also applying Gain)
    CalculateGeneralizedDEMA(rates_total, InpSlowDEMAPeriod, InpSignalPeriod, InpDemaGain, raw_macd_buffer, raw_signal_buffer);
