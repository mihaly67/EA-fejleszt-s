...
class ZMQReceiverThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            # CRITICAL FIX: Do NOT skip "HUD_INIT" messages if they arrive in the same burst as tick messages.
            latest_tick_data = None
            init_data = None
            while True:
                try:
                    ...
                    if msg.startswith("HUD_INIT "):
                        ...
                        json_data = msg[4:]
                        latest_tick_data = json.loads(json_data)
                except zmq.Again:
                    break
            ...
            # If we received an init message in this batch, emit it FIRST
            if init_data is not None:
                self.data_received.emit(init_data)

            # Then emit the latest tick if available
            if latest_tick_data is not None:
                self.data_received.emit(latest_tick_data)
    ...
    def on_data_received(self, data):
        ...
        if 'type' in data and data['type'] == 'init':
            try:
                ...
                history_list = []
                # Ensure we don't go out of bounds if lists mismatch slightly
                length = min(len(times), len(opens), len(highs), len(lows), len(closes))
                for i in range(length):
                    dt = pd.to_datetime(times[i], unit='s').strftime('%Y-%m-%d %H:%M:%S')
                    history_list.append({
                        'time': dt,
                        'open': opens[i],
                        'high': highs[i],
                ...
                if history_list:
                    ...
                    # Setup dummy subchart with same length so X-axis scales align perfectly
                    ...
                    print(f"Historical data ({length} bars) loaded successfully.")
            except Exception as e:
