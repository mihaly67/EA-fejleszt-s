...
class DualPaneHUD(QMainWindow):
    def __init__(self):
        ...
        # Using explicit series initialization for clean code and performance
    ...
    def on_data_received(self, data):
        ...
        try:
            if not self.is_initialized:
                # Lightweight-charts-python supports native dictionary initialization for extreme performance
                self.chart.set(pd.DataFrame([{'time': ts_str, 'open': self.current_open, 'high': self.current_high, 'low': self.current_low, 'close': price}]))
                self.subchart.set(pd.DataFrame([{'time': ts_str, 'open': 0.0, 'high': 1.0, 'low': 0.0, 'close': 0.5}]))

                self.p_long_line.set(pd.DataFrame([{'time': ts_str, 'P_Long': data.get('p_long', 0.0)}]))
                self.p_short_line.set(pd.DataFrame([{'time': ts_str, 'P_Short': data.get('p_short', 0.0)}]))
                self.p_noise_line.set(pd.DataFrame([{'time': ts_str, 'P_Noise': data.get('p_noise', 0.0)}]))

                self.dummy_min.set(pd.DataFrame([{'time': ts_str, 'DummyMin': 0.0}]))
                self.dummy_max.set(pd.DataFrame([{'time': ts_str, 'DummyMax': 1.0}]))
                self.is_initialized = True
            else:
                # Optimized High-Frequency UI Updates (Zero Pandas Overhead on active ticks)
                # Dictionary unpack update is native and avoids creating large Pandas objects in memory continuously
                self.chart.update({'time': ts_str, 'open': self.current_open, 'high': self.current_high, 'low': self.current_low, 'close': price})
                ...
                # Update Subchart (Predictions) natively using Python dictionaries to prevent CPU thrashing
                self.subchart.update({'time': ts_str, 'open': 0.0, 'high': 1.0, 'low': 0.0, 'close': 0.5})
