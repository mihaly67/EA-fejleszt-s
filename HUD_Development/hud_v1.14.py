...
class BasicHUD(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Jules HUD - v1.14 (Static Subchart + 4D Asym Thresholds)")
        ...
        # FIX: scaleMargins (top: 0.1, bottom: 0.1) hogy látszódjon minden címke
        js_code = f"""
        ...
        # Dinamikus görbék (A Lightweight charts-ban a create_line() önállóan nem jelenik meg vízszintes vonalként amíg nincs múltbéli adat)
        ...
        # 4D Optuna Aszimmetrikus Küszöbök (Thresholds) beállítása (a Copilot backend logja alapján)
        self.thresh_long = self.subchart.horizontal_line(0.35, color='rgba(0, 255, 0, 0.4)', width=1, style='dotted', text='Opt-L (0.35)', axis_label_visible=True)
        self.thresh_short = self.subchart.horizontal_line(0.36, color='rgba(255, 0, 0, 0.4)', width=1, style='dotted', text='Opt-S (0.36)', axis_label_visible=True)
        self.thresh_noise = self.subchart.horizontal_line(0.47, color='rgba(169, 169, 169, 0.4)', width=1, style='dotted', text='Opt-N (0.47)', axis_label_visible=True)
