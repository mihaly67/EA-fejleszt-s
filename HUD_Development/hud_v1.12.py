...
class BasicHUD(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Jules HUD - v1.12 (Static Subchart)")
        ...
        # FIX a statikus skálára (autoscaleInfoProvider)
        js_code = f"""
        {self.subchart.id}.chart.priceScale('right').applyOptions({{
            autoScale: false,
            scaleMargins: {{ top: 0.1, bottom: 0.1 }},
        }});
        // Hozzácsatoljuk az autoscaleInfoProvider-t az első szériához (Dummy módszer)
        """
        ...
        self.anchor_top = self.subchart.horizontal_line(1.0, color='rgba(255, 255, 255, 0.1)', width=1, style='dashed', axis_label_visible=False)
        self.anchor_bottom = self.subchart.horizontal_line(0.0, color='rgba(255, 255, 255, 0.1)', width=1, style='dashed', axis_label_visible=False)
