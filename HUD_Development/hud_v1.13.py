...
class BasicHUD(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Jules HUD - v1.13 (Live + History)")
        ...
        self.is_initialized = True  # Mivel már betöltöttük a chartot
    ...
    def load_historical_data(self):
        history_path = "/home/Jules/.wine/drive_c/Program Files/MetaTrader 5 IC Markets EU/MQL5/Files/history_init.csv"
        if not __import__('os').path.exists(history_path):
            print(f"Warning: Történelmi adat fájl nem található: {history_path}")
        ...
        try:
            ...
            df_history = df.tail(100).copy()
            df_history['time'] = pd.to_datetime(df_history['time'].astype(int), unit='s')

            self.chart.set(df_history)

            print(f"Sikeresen betöltve {len(df_history)} múltbeli gyertya a fő chartba.")
            self.is_initialized = True
        except Exception as e:
            print(f"Hiba a múltbeli adatok betöltésekor: {e}")
            self.is_initialized = False
