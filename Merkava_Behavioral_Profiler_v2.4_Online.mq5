...
void SendTickToPython(long time_msc, double bid, double ask, string pos_type_str, string pos_price_str, double pos_profit, long av1, long av2, long bv1, long bv2, double ap1, double ap2, double bp1, double bp2) {
    if(!g_dom_socket_connected) return;

    // Alap Vaku payload (rövidebb, hogy kompatibilis maradjon a Vaku3 kóddal)
    string payload_vaku = "TICK|" + IntegerToString(time_msc) + "|" + DoubleToString(bid, _Digits) + "|" + DoubleToString(ask, _Digits) + "|" + pos_type_str + "|" + pos_price_str + "|" + DoubleToString(pos_profit, 2) + "\n";

    // Teljes DOM payload a LGBM Tick Bridge számára
    string payload_dom = "TICK|" + IntegerToString(time_msc) + "|" + DoubleToString(bid, _Digits) + "|" + DoubleToString(ask, _Digits) + "|" + pos_type_str + "|" + pos_price_str + "|" + DoubleToString(pos_profit, 2) + "|" + IntegerToString(av1) + "|" + IntegerToString(av2) + "|" + IntegerToString(bv1) + "|" + IntegerToString(bv2) + "|" + DoubleToString(ap1, _Digits) + "|" + DoubleToString(ap2, _Digits) + "|" + DoubleToString(bp1, _Digits) + "|" + DoubleToString(bp2, _Digits) + "\n";
...
void CleanChartCompletely(long chart_id) {
    // 1. Delete ALL objects (lines, arrows, texts, panels), not just EA specific ones
    ObjectsDeleteAll(chart_id, -1, -1);
    ObjectsDeleteAll(chart_id, 0, -1); // Force main window cleanup too

    // 2. Remove ALL indicators from ALL windows (main chart + subcharts)
    int windows = (int)ChartGetInteger(chart_id, CHART_WINDOWS_TOTAL);
    for(int w=0; w<windows; w++) {
        int ind_total = ChartIndicatorsTotal(chart_id, w);
        // Iterate backwards so deletion doesn't mess up indices
    ...
    // 3. Turn off historical trade arrows/markers permanently
...
int OnInit()
{
   CleanChartCompletely(0);
   m_nav_system.Release();

   // --- SEND 1-DAY HISTORICAL BARS TO PYTHON MACRO PORT (5555) ---
   if(InpEnablePythonBridge) {
       int hist_socket = SocketCreate();
       if(hist_socket != INVALID_HANDLE) {
           if(SocketConnect(hist_socket, InpBridgeHost, InpBridgePort, 1000)) { // Give it 1 second timeout to connect
               MqlRates rates[];
               ArraySetAsSeries(rates, true);
               int copied = CopyRates(_Symbol, PERIOD_M1, 0, 1440, rates);

               if(copied > 0) {
                   string payload = "{\"type\":\"init\", \"times\":[";
                   for(int i=copied-1; i>=0; i--) { payload += IntegerToString(rates[i].time) + (i==0 ? "" : ","); }
                   payload += "], \"opens\":[";
                   for(int i=copied-1; i>=0; i--) { payload += DoubleToString(rates[i].open, _Digits) + (i==0 ? "" : ","); }
                   payload += "], \"highs\":[";
                   for(int i=copied-1; i>=0; i--) { payload += DoubleToString(rates[i].high, _Digits) + (i==0 ? "" : ","); }
                   payload += "], \"lows\":[";
                   for(int i=copied-1; i>=0; i--) { payload += DoubleToString(rates[i].low, _Digits) + (i==0 ? "" : ","); }
                   payload += "], \"closes\":[";
                   for(int i=copied-1; i>=0; i--) { payload += DoubleToString(rates[i].close, _Digits) + (i==0 ? "" : ","); }
                   payload += "]}\n";

                   uchar buf[];
                   StringToCharArray(payload, buf);
                   SocketSend(hist_socket, buf, ArraySize(buf)-1);
                   Print("Historical init payload (1440 bars) sent successfully to Python on port ", InpBridgePort);
               } else {
                   Print("Failed to copy historical rates.");
               }
               SocketClose(hist_socket);
           } else {
               Print("Failed to connect to Python for history push on port ", InpBridgePort);
               SocketClose(hist_socket);
           }
       }
   }
   ...
   // Open CSV for LGBM Predictions
   string csv_name = "LGBM_Live_Predictions_" + TimeToString(TimeCurrent(), TIME_DATE|TIME_MINUTES) + ".csv";
...
void SendMacroToPython(double current_price) {
    ...
    // Get current M1 candle data to send to Python so it can calculate ZigZag
    double high[], low[];
    datetime time[];
    ...
    if(!g_zigzag_init_sent) {
        int init_bars = 1440; // 24 hours of M1 data for stable ZigZag init
        if(CopyHigh(_Symbol, PERIOD_M1, 0, init_bars, high) > 0 && CopyLow(_Symbol, PERIOD_M1, 0, init_bars, low) > 0 && CopyTime(_Symbol, PERIOD_M1, 0, init_bars, time) > 0) {
            ...
            for(int i=0; i<ArraySize(time); i++) {
                json += IntegerToString(time[i]);
                if(i < ArraySize(time) - 1) json += ",";
            }
            json += "]}\n";
            ...
            if(SocketSend(g_socket, buffer, ArraySize(buffer) - 1) >= 0) {
                g_zigzag_init_sent = true;
                Print("✅ Sent ", init_bars, " M1 candles for Python ZigZag Init.");
            } else {
        ...
        if(CopyHigh(_Symbol, PERIOD_M1, 0, 1, high) > 0 && CopyLow(_Symbol, PERIOD_M1, 0, 1, low) > 0 && CopyTime(_Symbol, PERIOD_M1, 0, 1, time) > 0) {
            ...
            if(SocketSend(g_socket, buffer, ArraySize(buffer) - 1) < 0) {
                g_socket_connected = false;
                g_zigzag_init_sent = false; // Re-init next time
...
void OnTick()
{
   ...
   if(InpEnablePythonBridge) {
       ...
       if(g_dom_socket_connected) {
           string pos_type_str = "0";
           string pos_price_str = "0.0";
               ...
               for(int i=0; i<PositionsTotal(); i++) {
                   if(m_position.SelectByIndex(i)) {
                       if(m_position.Symbol() == _Symbol) {
                           if(first_pos) {
                               pos_price_str = DoubleToString(m_position.PriceOpen(), _Digits);
                               pos_type_str = (m_position.PositionType() == POSITION_TYPE_BUY) ? "1" : "-1";
           ...
           SendTickToPython(tick.time_msc, tick.bid, tick.ask, pos_type_str, pos_price_str, pos_profit, av1, av2, bv1, bv2, ap1, ap2, bp1, bp2);
       }
   }
